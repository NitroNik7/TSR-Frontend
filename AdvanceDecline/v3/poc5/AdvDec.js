...
// Ultra-modern, sleek floating tooltip style
let tooltipStyle = ` 
	display: block;
	position: absolute; 
	border: none; 
...
// Beautiful, vibrant line styles with smooth joints
var advancesLineStyle = 'fill: none; stroke-width: 2.5; stroke: #10b981; stroke-linecap: round; stroke-linejoin: round; filter: drop-shadow(0px 2px 4px rgba(16, 185, 129, 0.15));';
var declinesLineStyle = 'fill: none; stroke-width: 2.5; stroke: #f43f5e; stroke-linecap: round; stroke-linejoin: round; filter: drop-shadow(0px 2px 4px rgba(244, 63, 94, 0.15));';

// Interactive pointer circles (halo effect)
var circleAdvancesStyle = 'fill: #10b981; stroke: #ffffff; stroke-width: 2; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.15));';
var circleDeclinesStyle = 'fill: #f43f5e; stroke: #ffffff; stroke-width: 2; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.15));';
let circleRadius = 5; 

// Clean, minimal crosshair vertical guide
...
function drawChartFromData(data) {
	...
	svg.append("g")
		...
		.call(d3.axisBottom(x).ticks(xAxisTickFreq))
	...
	let circleDeclines = svg.append("g")
		...
		.attr("transform", "translate(" + margin.left + "," + margin.top + ")") // Fixed align transform from 0 to margin.top
		.attr("opacity", 0);

	// Crosshair vertical layout line
	svg.append("line")
		.attr("id", "crosshair")
		.attr("x1", 0)
		.attr("y1", 0)
	...
	// Hidden overlay rect for capturing full-width track actions
	svg.append("rect")
		.attr("id", "rectOverlay")
		.attr("width", width)
		.attr("height", height)
	...
	function mouseover(e) {
		...
		div.style.opacity = "1";
		d3.select("#crosshair").attr("opacity", "1");
	...
	function mousemove(e) {
		...
		if (index <= data.length - 1) {
			...
			// Smooth tooltip placement side switching
			if (xCord > width / 2) { 
				div.style.left = (e.clientX - div.getBoundingClientRect().width - 15) + "px";
			} else {
			...
			div.innerHTML = `
				<div style="font-weight: 600; color: #0f172a; margin-bottom: 6px; border-bottom: 1px solid #f1f5f9; padding-bottom: 4px;">
					${displayDate}
				</div>
