...
// TODO Add Header to showSuggestions Div

// ! Issue #1 Advanced options not getting updated correctly
...
let stockMetrics = {
  "price": {
    "fields": ["open", "high", "low", "close", "o1", "o2", "h1", "l1"],
    "compFields": {
      "fields": ["open", "high", "low", "close", "o1", "o2", "h1", "l1"],
      "overlays": ["Price & Bollinger", "Bollinger Sequence", "Ichimoku (old)", "Price & Keltner", "PSAR"],
      "movingAverages": ["SMA", "EMA", "ZLEMA"]
    }
  },
  "funda": {
    "fields": ["Market_Cap", "Operating_Profit", "Price_to_Earning"],
    "compFields": {
      "fields": ["Market_Cap", "Operating_Profit", "Price_to_Earning"]
    }
  },
...
function showSuggestions(e) {
  ...
  if ((/[a-zA-Z]/).test(keyCode) && keyCode.length == 1) {
    ...
    let header = document.createElement("section");
    header.innerHTML = "Related Fields: ";
    header.style.fontSize = "12px";
    header.style.fontWeight = "bold";
    header.style.backgroundColor = "grey";
    header.style.color = "white";
    ...
    Object.entries(stockMetrics).forEach(metric => {
      ...
      Object.entries(metric[1].compFields).forEach(field => {
        console.log(field, field[0])
        header = document.createElement("section");
        header.innerHTML = field[0];
        header.style.fontSize = "12px";
        header.style.fontWeight = "bold";
        header.style.backgroundColor = "grey";
        header.style.color = "white";
