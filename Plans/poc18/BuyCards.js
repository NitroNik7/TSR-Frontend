...
var miSupc = (function () { // Mi Subscription plan cards ...
    ...
    */
    ...
    let html = `<div class="tsrPlanCard ${popular}" data-plan-id="${details.id}">`;

    // Popular Crown Ribbon
    if (details.pop) {
        html += `<span class="tsrPlanPopularTag"><i class="fas fa-crown"></i> Popular</span>`;
    }

    // Top Section (Headers, Price, Target Audience)
    html += `  <div class="tsrPlanCardTop">`;
    html += `    <h3 class="tsrPlanCardTitle">${details.name}</h3>`;

    // Strike-through Original Price
    if (jsu.isNotNull(planData.off) || jsu.isNotNull(renewDiscount) || jsu.isNotNull(upgradeDiscount) || (curSub && curSub.valueRemain)) {
        html += `    <div class="tsrPlanOldPrice"><i class="fas fa-rupee-sign"></i>${origCost}</div>`;
    } else {
        html += `    <div class="tsrPlanOldPrice"></div>`;
    }

    // Active Buy Price
    let planPeriodText = (period === "1M") ? "month" : (planData.period !== "1 Year" ? planData.period : "year");
    html += `    <h2 class="tsrPlanMainPrice">`;
    html += `      <i class="fas fa-rupee-sign tsrPlanPriceCurrency"></i>${buyPrice}`;
    html += `      <span class="tsrPlanPricePeriod">/${planPeriodText}</span>`;
    html += `    </h2>`;

    // Effective Rate Per Day
    if (jsu.isNotNull(planData.off)) {
        html += `    <div class="tsrPlanEffectivePrice">`;
        html += `      Effective <strong><i class="fas fa-rupee-sign"></i> ${Math.round(planData.perMth / 30)}/day</strong>`;
        html += `    </div>`;
    }

    // Savings Tag
    if (jsu.isNotNull(savings) && savings > 0) {
        html += `    <div class="tsrPlanSavingsWrap">`;
        html += `      <p class="tsrPlanSavingsBadge">Save <i class="fas fa-rupee-sign"></i>${savings}</p>`;
        html += `    </div>`;
    }

    html += `    <hr class="tsrPlanDivider">`;

    // Target Audience (Suitable For)
    if (details.fit) {
        html += `    <div class="tsrPlansFitCardContainer p-2 p-sm-3 rounded-3 bg-white border shadow-sm my-1">`;
        html += `      <p class="tsrPlanCardFitDescription m-0 d-flex align-items-center">`;
        html += `        <i class="fas fa-user-check me-2 tsrPlansFitIcon"></i>`;
        html += `        <span>Suitable for <span style="color: #2563eb; font-weight: 600;">${details.fit}</span></span>`;
        html += `      </p>`;
        html += `    </div>`;
    }
    ...
    if (details.entitlements && details.entitlements.length > 0) {
        html += `    <ul class="tsrPlanFeaturesList">`;
        for (let i = 0; i < details.entitlements.length; i++) {
            html += `      <li class="tsrPlanFeatureItem">`;
            html += `        <i class="fas fa-check tsrPlanFeatureCheck"></i>`;
            html += `        <span>${details.entitlements[i]}</span>`;
            html += `      </li>`;
        }
        html += `    </ul>`;
    }

    html += `  </div>`; // end tsrPlanCardTop

    // Bottom Section (Action Button / Notice)
    html += `  <div class="tsrPlanCardBottom">`;

    if (jsu.isNull(curPlan) || curPlan == 'EXPIRED') {
        html += `    <button class="tsrPlanActionBtn" onclick="miSupc.pa('buy','${details.id}', '${period}');">Buy</button>`;
    } else {
        if (state == 'disable') {
            html += `    <span class="tsrPlanStatusNotice tsrNoticeInfo">You already have a higher plan</span>`;
        } else if (state == 'renew') {
            html += `    <button class="tsrPlanActionBtn" onclick="miSupc.pa('renew', '${details.id}', '${period}');">Renew</button>`;
        } else {
            if (buyPrice < 0) {
                html += `    <span class="tsrPlanStatusNotice tsrNoticeWarning">Current value exceeds plan price. Select higher period</span>`;
            } else {
                html += `    <button class="tsrPlanActionBtn" onclick="miSupc.pa('upgrade', '${details.id}', '${period}');">Upgrade</button>`;
            }
        }
    }

    html += `  </div>`; // end tsrPlanCardBottom
    html += `</div>`;   // end tsrPlanCard
