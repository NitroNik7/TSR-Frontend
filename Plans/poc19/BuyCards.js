...
var miSupc = (function () { // Mi Subscription plan cards ...
    ...
    let CSS_OLD_PRICE = `text-decoration: line-through; color: #adb5bd; font-size: 16px; text-wrap: nowrap; text-decoration-thickness: 2px; text-decoration-color: #a8a8a8;`

    // let CSS_DISCOUNT = `background: #e6f4ea;color: #2f9e44;padding: 3px 8px; border-radius: 999px;font-size: 11px;font-weight: 600;`
    // let CSS_DISCOUNT = `background: #e6f4ea;color: #2f9e44;padding: 3px 8px; border-radius: 999px;font-size: 11px;font-weight: 600;`
    let CSS_SAVINGS = `background: #e6f4ea; color: #2f9e44; padding: 5px 8px; border-radius: 999px; font-size: 12px; font-weight: bold; width: max-content; margin: auto;`

    let CSS_PRICE = `font-size: 44px;font-weight: 600;margin-bottom: 6px; display: flex; align-items: baseline; margin: 0 auto; gap: 10px;`
    let CSS_PRICE_SPAN = `font-size: 14px;color: #6c757d;`
    let CSS_EFF_PRICE = `font-size: 14px; color: #6c757d; margin-top: 8px; margin-bottom: 16px;`
    ...
    */
    ...
    function getCards(period) {
        ...
        // html += getCard(jsu.getObjFrmArr(planDetails, "TRADER"), period);
    ...
    //             <div style="display: flex; flex-direction: column; justify-content: space-between" data-plan-id="${details.id}">`

    //     if (jsu.isNotNull(popular)) {
    //         html += `   <span class="badge btn-warning" style="position: absolute; top: -12px; left: 36%;  border-radius: 10px"><i class="fas fa-crown"></i> Popular</span>`
    //     }
    ...
    //         html += `   <i class="fas fa-rupee-sign" style="font-size: 24px;"></i> ${buyPrice}`
    //         if (period == "1M") {
    ...
    //     let isFeatured = Boolean(details.pop);
    //     let renewDiscount = details.renew;
    //     let upgradeDiscount = details.upgrade;
    ...
    //     } else {
    //         buyPrice = planData.buyPrice;
    //         savings = planData.orig - planData.buyPrice;
    //     }
    ...
    //     let durationSuffix = '/yr';
    //     let periodText = 'annually';
    //     if (period == '1M') {
    //         durationSuffix = '/mo';
    //         periodText = 'monthly';
    //     } else if (planData.period && planData.period != '1 Year') {
    //         durationSuffix = `/ ${planData.period}`;
    //         periodText = `per ${planData.period}`;
    //     }

    //     let dailyRate = 0;
    //     if (jsu.isNotNull(planData.perMth)) {
    //         dailyRate = Math.round(planData.perMth / 30);
    //     } else {
    //         dailyRate = Math.round(buyPrice / (period == '1M' ? 30 : 365));
    //     }

    //     let discountPercent = planData.off || 0;
    //     if (state == 'renew' && renewDiscount) {
    //         discountPercent += renewDiscount;
    //     } else if (state == 'upgrade' && upgradeDiscount) {
    //         discountPercent += upgradeDiscount;
    //     }

    //     let featuredCardClass = isFeatured ? 'tsrPlansCardFeatured' : '';

    //     let html = `
    //         <div class="tsrPlansCardCol">
    //         <div class="card h-100 tsrPlansCard ${featuredCardClass}" data-plan-id="${details.id}" id="tsrPlansCard${details.id}">
    //             <div class="card-body d-flex flex-column justify-content-between p-3 p-xl-4">
    //             <div class="tsrPlansHeaderSection">

    //                 <div class="d-flex align-items-center justify-content-between mb-2">
    //                 <h3 class="tsrPlansCardHeading m-0">${details.name}</h3>
    //                 </div>

    //                 <!-- Focused Price Hero Box -->
    //                 <div class="tsrPlansPriceHeroBox text-center p-3 rounded-3 mb-3">`;

    //     if (origCost && (savings > 0 || discountPercent > 0)) {
    //         html += `
    //             <div class="d-flex align-items-center justify-content-center gap-2 mb-1">
    //                 <span class="tsrPlansStrikethroughPrice"><i class="fas fa-rupee-sign"></i>${origCost}</span>`;
    //         if (discountPercent > 0) {
    //             html += `
    //                 <span class="tsrPlansDiscountPercentBadge">${discountPercent}% OFF</span>`;
    //         }
    //         html += `
    //             </div>`;
    //     }
    ...
    //     html += `
    //             <div class="tsrPlansMainPriceWrap d-flex align-items-baseline justify-content-center">
    //                 <span class="tsrPlansCurrency"><i class="fas fa-rupee-sign"></i></span>
    //                 <span class="tsrPlansPriceFigure">${buyPrice}</span>
    //                 <span class="tsrPlansDuration">${durationSuffix}</span>
    //             </div>

    //             <div class="tsrPlansDailyMicroCost mt-2">
    //                 <span class="tsrPlansDailyPill">
    //                 <i class="fas fa-bolt text-warning me-1"></i> Just <strong>₹${dailyRate} / day</strong>
    //                 </span>
    //             </div>`;

    //     if (savings > 0) {
    //         html += `
    //             <div class="tsrPlansSavingsMeta mt-2">
    //                 <span>You save ₹${savings} ${periodText}</span>
    //             </div>`;
    //     }
    ...
    //             <!-- Audience Fit -->
    //             <div class="tsrPlansSuitabilityBox p-2 rounded-2 mb-3">
    //             <i class="fas fa-bullseye text-primary me-2 tsrPlansTargetIcon"></i>
    //             <span>Suitable for </span>`;

    //     if (details.fit && details.fit.length > 0) {
    //         html += `<ul>`;
    //         for (let i = 0; i < details.fit.length; i++) {
    //             html += `<li>${details.fit[i]}</li>`;
    //         }
    //         html += `</ul>`;
    //     }
    ...
    //         <!-- Features -->
    //         <ul class="tsrPlansFeatureItemList list-unstyled mb-4">`;

    //     if (details.entitlements && details.entitlements.length > 0) {
    //         for (let i = 0; i < details.entitlements.length; i++) {
    //             html += `
    //           <li class="d-flex align-items-start mb-2">
    //             <i class="fas fa-check-circle text-success me-2 mt-1"></i>
    //             <span>${details.entitlements[i]}</span>
    //           </li>`;
    //         }
    //     }
    ...
    //       <!-- CTA Container -->
    //       <div class="tsrPlansCtaContainer mt-auto pt-2">`;

    //     let buttonClass = isFeatured ? 'btn btn-primary tsrPlansButtonFeatured' : 'btn btn-outline-primary';

    //     if (jsu.isNull(curPlan) || curPlan == 'EXPIRED') {
    //         html += `
    //                     <button class="${buttonClass} w-100 tsrPlansButton" onclick="miSupc.pa('buy', '${details.id}', '${period}');" id="tsrPlansBtn${details.id}">
    //                     Buy Plan
    //                     </button>`;
    //     } else {
    //         if (state == 'disable') {
    ...
    //         } else if (state == 'renew') {
    //             html += `
    //                     <button class="${buttonClass} w-100 tsrPlansButton" onclick="miSupc.pa('renew', '${details.id}', '${period}');" id="tsrPlansBtn${details.id}">
    //                     Renew Plan
    //                     </button>`;
    //         } else {
    //             if (buyPrice < 0) {
    ...
    //             } else {
    //                 html += `
    //                     <button class="${buttonClass} w-100 tsrPlansButton" onclick="miSupc.pa('upgrade', '${details.id}', '${period}');" id="tsrPlansBtn${details.id}">
    //                     Upgrade Plan
    //                     </button>`;
    //             }
    //         }
    ...
    let effectivePerMonth = 0;
    if (period == '1M') {
        effectivePerMonth = buyPrice;
    } else if (jsu.isNotNull(planData.perMth)) {
        effectivePerMonth = Math.round(planData.perMth);
    } else {
        effectivePerMonth = Math.round(buyPrice / 12);
    }
    ...
    let html = `
        <div class="tsrPlansCardCol">
            <div class="card h-100 tsrPlansCard position-relative ${featuredCardClass}" data-plan-id="${details.id}" id="tsrPlansCard${details.id}">`;

    // Flagship Badge for Featured Plan
    if (isFeatured) {
        html += `
                <div class="tsrPlansTopRibbon">
                    <span><i class="fas fa-crown me-1"></i> MOST POPULAR</span>
                </div>`;
    }
    ...
    if (period != '1M') {
        html += `
                                    <strong>₹${effectivePerMonth}</strong>/mo <span class="tsrPlansEffectiveDivider">•</span> `;
    }

    html += `
                                    <i class="fas fa-bolt text-warning me-1"></i><strong>₹${dailyRate}</strong>/day
                                </span>
                            </div>`;
    ...
    html += `
                            <div class="tsrPlansTaxNotice mt-1">
                                <span>Incl. of all taxes</span>
                            </div>
                        </div>

                        <!-- Audience Fit (Clean Tag Chips) -->
                        <div class="tsrPlansSuitabilityBox p-2 rounded-2 mb-3">
                            <div class="tsrPlansSuitabilityHeader d-flex align-items-center mb-2">
                                <i class="fas fa-bullseye text-primary me-2 tsrPlansTargetIcon"></i>
                                <span>Suitable for</span>
                            </div>`;
    ...
    let buttonClass = isFeatured ? 'btn btn-primary tsrPlansButtonFeatured' : 'btn btn-outline-primary';
    let buttonArrow = isFeatured ? ' <i class="fas fa-arrow-right ms-1"></i>' : '';

    if (jsu.isNull(curPlan) || curPlan == 'EXPIRED') {
        html += `
                        <button class="${buttonClass} w-100 tsrPlansButton" onclick="miSupc.pa('buy', '${details.id}', '${period}');" id="tsrPlansBtn${details.id}">
                            Buy Plan${buttonArrow}
                        </button>`;
    } else {
        if (state == 'disable') {
            html += htmlU.getSpan('You already have higher plan', 'cornflowerblue', 12);
    ...
    // Carousel Pagination Indicators (<992px)
    function initTsrPlansCarousel() {
        const container = document.getElementById('tsrPlansCards');
        const dotsContainer = document.getElementById('tsrPlansCarouselDots');
        if (!container || !dotsContainer) return;

        const cards = container.querySelectorAll('.tsrPlansCardCol');
        if (cards.length <= 1) {
        ...
        cards.forEach((_, index) => {
            ...
            dot.className = `tsrPlansDot ${index === 0 ? 'tsrPlansDotActive' : ''}`;
        ...
        const dots = dotsContainer.querySelectorAll('.tsrPlansDot');
            ...
            scrollTimeout = setTimeout(() => {
                ...
                cards.forEach((card, index) => {
                    ...
                    if (scrollCenter >= cardLeft && scrollCenter <= cardRight) {
                        dots.forEach((d) => d.classList.remove('tsrPlansDotActive'));
                        if (dots[index]) dots[index].classList.add('tsrPlansDotActive');
                    }
                });
            }, 50);
        });
    ...
    document.addEventListener('DOMContentLoaded', initTsrPlansCarousel);

    // Carousel Dots & Viewport Sync (<992px)
    function initTsrPlansCarousel() {
        const container = document.getElementById('tsrPlansCards');
        const dotsContainer = document.getElementById('tsrPlansCarouselDots');
        if (!container || !dotsContainer) return;
    ...
    // Run indicator sync after plans render
    document.addEventListener('DOMContentLoaded', initTsrPlansCarousel);
