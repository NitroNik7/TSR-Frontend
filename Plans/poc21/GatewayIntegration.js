	...
	function userAction(action, id, period, country, telPrefix, tel) {
		...
		if (action == 'BUY_AIO') {
			...
			if (jsu.isNull(curPlan) || curPlan == 'EXPIRED' || curPlan == 'BASIC') {
				...
				let msg = 'This Offer is applicable only for TSR Premium Users';
				mintHtmlUtil.addMsgToDiv('aioFBDiv', true, msg, 'red', 12);;
				return;
			}
		}
		...
		if (action == 'NEW' || action == 'UPGRADE' || action == 'RENEW'
			...
			let postData = {
				reqType: action, subType: id, period: period, referral: referral, payGate: payGate,
				country: country, telPrefix: telPrefix, tel: tel, aioIncluded: aioIncluded,

			}
			
			// TODO 
			if (action == 'NEW' || action == 'UPGRADE' || action == 'RENEW'){
				let aioCheck = document.getElementById("tsrPlanStockAioCheck").checked;
				var aioIncluded = aioCheck ? true : false;
				postData.aioIncluded = aioIncluded;
			}
			...
			$.ajax({
				url: MY_SP_URL, type: "POST", data: postData,
				success: function (result) {
					if (result.statusCode == MSG_STATUS_GOOD) {
						...
						htmlU.addMsgToDiv(SP_FEEBBACK_DIV, true, BR_2 + BR_2 + doBold(result.statusMsg), 'red', 12);
						...
						*/
						...
						htmlU.addMsgToDiv('subBuyMsgDiv', true, BR_2 + BR_2 + htmlU.doBold(result.statusMsg), 'red', 12);
