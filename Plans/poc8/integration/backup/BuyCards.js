var miSupc = (function () { // Mi Subscription plan cards ...
    ...
    function getCards(period) {
        ...
        if (jsu.isNotNull(curPlan)) {
            let planDef = jsu.getObjFrmArr(planDetails, curPlan);
            let planData = jsu.getObjFrmArr(planDef.period, DEF_PERIOD);
            curPlanCost = planData.orig;
        }
    ...
    function getCard(details, period) {
        ...
        if (jsu.isNotNull(curPlan)) {
            if (planData.orig < curPlanCost) {
                state = 'disable'
            } else if (planData.orig == curPlanCost) {
                state = 'renew'
            } else {
                state = 'upgrade'
            }
        ...
        if (jsu.isNotNull(planData.off)) {
            ...
            if (state == 'renew' && jsu.isNotNull(renewDiscount)) {
                ...
                html += `   <span style="${CSS_DISCOUNT}" class="">Renew Discount ${renewDiscount}% OFF</span>`
                ...
                html += `       Current Value <i class="fas fa-rupee-sign"></i> ${curSub.valueRemain} OFF`
                html += `   </span>`
            }
            html += `</div>`
        }
    ...
    function planAction(action, detailsId, period) {
        ...
        // TODO put in SERVER SIDE HTML
        let planActionModal = document.getElementById(planActionModalBodyId);
        if (jsu.isNull(planActionModal)) {
            let div = document.createElement("div");
            div.id = "tsrTempDiv";

            // action = action.replace(action.chartAt(0), action.chartAt(0).toUpperCase());
            div.innerHTML = `<div class="modal fade" id="tsrPlansActionModal" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-lg modal-dialog-centered">
                                <div class="modal-content border-0 shadow">
                                    <div class="modal-header bg-light">
                                        <h5 class="modal-title" id="tsrPlansActionModalTitle">${action} Plan</h5>
                                        <button type="button" class="btn-close" data-bs-toggle="modal"
                                            data-bs-target="#tsrPlansActionModal" aria-label="Close"></button>
                                    </div>

                                    <div class="modal-body p-4" id="tsrPlansActionModalBody">
    ...
    function buyPlanModalHtml(detailsId, period) {
        ...
        html += `    <button  onsubmit="return false;" onclick="miSupc.hs('buy' ,'${detailsId}', '${period}')" class="btn btn-primary w-100">PROCEED TO PAYMENT</button>`
        // html += `</form>`
    ...
    function upgradePlanModalHtml(detailsId, period) {
        ...
        if (jsu.isNotNull(upgradeDiscount)) {
            buyPrice = planData.orig - (((planData.off + upgradeDiscount) / 100) * planData.orig);
        } else {
            buyPrice = planData.buyPrice;
        }
        buyPrice = Math.round(buyPrice);
    ...
    function handleSubmit(action, detailsId, period) {
        ...
        if (form.reportValidity()) {
            ...
            let country = countryDetails[0].trim();
            let telPrefix = countryDetails[1].trim();
