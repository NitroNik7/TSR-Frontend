var miSrg = (function () {
    ...
    function calcRrg(idxData) {
        ...
        const m = 14; // Smoothing & rolling window period
        const k = 10; // ROC lookback period
        const alpha = 2.0 / (m + 1); // EMA smoothing factor

        // Helper: Rolling Mean calculation
        ...
        // Helper: Rolling Sample Standard Deviation calculation
        ...
        // --- Step 2: Calculate JdK_RS (EMA Smoothing of RS) ---
        let jdkRsArr = [];
        jdkRsArr[0] = rsArr[0];
        for (let i = 1; i < rsArr.length; i++) {
            jdkRsArr[i] = (alpha * rsArr[i]) + ((1 - alpha) * jdkRsArr[i - 1]);
        }

        // --- Step 3: Calculate RS_Ratio (Z-Score of JdK_RS * 10 + 100) ---
        let rsRatioArr = [];
        for (let i = 0; i < dateKeys.length; i++) {
            let startIndex = Math.max(0, i - m + 1);
            let count = i - startIndex + 1;

            let meanJdkRs = calculateMean(jdkRsArr, startIndex, count);
            let stdDevJdkRs = calculateStdDev(jdkRsArr, startIndex, count, meanJdkRs);

            let rsRatio = 100.0;
            if (stdDevJdkRs > 0) {
                rsRatio = 100.0 + 10.0 * ((jdkRsArr[i] - meanJdkRs) / stdDevJdkRs);
            }

            rsRatioArr[i] = rsRatio;
            idxData[dateKeys[i]].rsRatio = rsRatio;
        }

        // --- Step 4: Calculate ROC (Rate of Change of JdK_RS over k lookback) ---
        let rocArr = [];
        for (let i = 0; i < dateKeys.length; i++) {
            let lookbackIdx = Math.max(0, i - k);
            let prevJdkRs = jdkRsArr[lookbackIdx];

            if (prevJdkRs !== 0) {
                rocArr[i] = (jdkRsArr[i] - prevJdkRs) / prevJdkRs;
            } else {
                rocArr[i] = 0.0;
            }
        }

        // --- Step 5: Calculate JdK_ROC (EMA Smoothing of ROC) ---
        let jdkRocArr = [];
        jdkRocArr[0] = rocArr[0];
        for (let i = 1; i < dateKeys.length; i++) {
            jdkRocArr[i] = (alpha * rocArr[i]) + ((1 - alpha) * jdkRocArr[i - 1]);
        }

        // --- Step 6: Calculate RS_Momentum (Z-Score of JdK_ROC * 10 + 100) ---
        for (let i = 0; i < dateKeys.length; i++) {
            let startIndex = Math.max(0, i - m + 1);
            let count = i - startIndex + 1;

            let meanJdkRoc = calculateMean(jdkRocArr, startIndex, count);
            let stdDevJdkRoc = calculateStdDev(jdkRocArr, startIndex, count, meanJdkRoc);

            let rsMomentum = 100.0;
            if (stdDevJdkRoc > 0) {
                rsMomentum = 100.0 + 10.0 * ((jdkRocArr[i] - meanJdkRoc) / stdDevJdkRoc);
            }

            idxData[dateKeys[i]].rsMomentum = rsMomentum;
        }
