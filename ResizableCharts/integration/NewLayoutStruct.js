var chnglost = (function () {   // chart NG Chart Layout Struct
    ...
    let chContainerId = "chartNgWrap";
    let chContainer;

    let horiContPrefix = "container";
    let vertContPrefix = "secondCol";
    ...
    function paintLayout(layoutHoriDivsParam, panelVertDivsParam) {
        chContainer = $("#" + chContainerId);

        layoutHoriDivs = layoutHoriDivsParam;
        panelVertDivs = panelVertDivsParam;


        htmlU.addCssToHead("row_resizer", css);
        chContainer.height("100%");

        createLayout(chContainer, horiContPrefix, "horizontal", layoutHoriDivs, true);
        ...
        if (layout == "vertical") { // vertical layout
            ...
            for (let i = 0; i < noOfDivs; i++) {
                ...
                if (i == 0) {
                    ...
                    element.append(`
                        <div class='column' id="${colId}" style='background-color: white; width: ` + (firstCol * element.width()) + `px; flex-grow: 1;'>
    ...
    function updateLayout(newlayout) {
        ...
        resetHoriCols(newlayout)
        resetVertRows(newlayout);

    }

    function resetVertRows(newlayout) {
        // let resizerHeight = 3;  // should match with .row_resizer CSS
        // let rowHeight = (element.height() - (noOfDivs - 1) * resizerHeight) / noOfDivs;
        let height = newlayout ? 3 : 0.001;
        for (let i = 0; i < panelVertDivs.length; i++) {
            let resizerId = vertContPrefix + "RowResizer" + i;
            $("#" + resizerId).height(height);
            // secondColRowCollapseBtn1
            let collapseBtnId = vertContPrefix + "RowCollapseBtn" + i;
            if (newlayout) {
                $("#" + collapseBtnId).show();
            } else {
                $("#" + collapseBtnId).hide();
    ...
    function resetHoriCols(newlayout) {
        let firstCol = .70;
        let secondCol = .3;
        ...
        // containerColResizer1
        // const chContainer = $("#" + chContainerId);
        let chContWidth = chContainer.width();

        elementWidth = (secondCol * chContWidth) - (layoutHoriDivs.length - 1) * resizerWidth;

        for (let i = 0; i < layoutHoriDivs.length; i++) {

            let divName = $("#" + layoutHoriDivs[i].id);
            if (i == 0) {
                divName.width(chContWidth * firstCol);
            } else {
                divName.width(elementWidth);
                let resizerId = horiContPrefix + "ColResizer" + i;
                $("#" + resizerId).width(resizerWidth);
                let collapseBtnId = horiContPrefix + "ColCollapseBtn" + i;
                if (newlayout) {
                    $("#" + collapseBtnId).show();
                } else {
                    $("#" + collapseBtnId).hide();
