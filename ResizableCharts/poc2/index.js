...
const container = $(".tsrChContainer");
...
let wlBoxId = "tsrChWlBox";
let chPanelBarId = "tsrChPanelBar";
...
function chooseLayout(id) {
    ...
    if (id == 9) {
        let openBoxes = addBtnsToPanelBar(panelBtns, "secondColRow");
        ...
        if (openBoxes.length > 0) {
            ...
            for (let i = 0; i < openBoxes.length; i++) {
                ...
                let html = getSidePanelHtml(infoBox);
        ...
        addOtherSettingsToPanelBar();
    }
}

function addBtnsToPanelBar(btns, boxEleIdPrefix) {
    ...
    htmlU.addMsgToDiv(chPanelBarId, true, html);
...
function getSidePanelHtml(infoBox) {
    let btnId = infoBox.btnId;
    let rowId = infoBox.rowId;
...
function addOtherSettingsToPanelBar() {
    let html = "";
    html += `<button class="btn btn-lg" title="Add alerts">`
    html += `   <i class="fas fa-bell"></i>`
    html += `</button>`

    let div = $("#" + chPanelBarId);
    div.append(html);
}
