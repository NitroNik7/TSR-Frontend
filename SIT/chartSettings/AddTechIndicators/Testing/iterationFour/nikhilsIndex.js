...
let activeBalanceSheetFields = [];
let activeCashFlowFields = [];
let activeIncomeStatementFields = [];
let activeQuarterlyIncomeFields = [];

// function to set default values for funda fields
(function () {
    if (fundaFields.length != 0) {
        fundaFields.forEach((subCategory, index) => {
            let subCategoryName = Object.keys(fundaFields[index])[0];
            if (index == 0) {
                activeBalanceSheetFields = new Array(subCategory[subCategoryName].length).fill(0);
            } else if (index == 1) {
                activeCashFlowFields = new Array(subCategory[subCategoryName].length).fill(0);
            } else if (index == 2) {
                activeIncomeStatementFields = new Array(subCategory[subCategoryName].length).fill(0);
            } else if (index == 3) {
                activeQuarterlyIncomeFields = new Array(subCategory[subCategoryName].length).fill(0);
            }
        });
    }
})();
    ...
    if (fundaFields.length != 0) {
        fundaFields.forEach((subCategory, index) => {
            ...
            if (index == 0)
                showFundaFields(0);
            div.innerHTML = `
            <button onclick="showFundaFields(${index});" style="background-color: white;">${subCategoryName}</button>
            `;
...
function showFundaFields(subCategoryIndex) {
    searchIndicatorInput.removeEventListener("input", search);
    subCategoriesWrapper.style.display = "flex";

    selectElement.innerHTML = "";
    let subCategoryName = Object.keys(fundaFields[subCategoryIndex])[0]; // to get the key name
    // console.log(fundaFields[subCategoryIndex]);
    let fields = fundaFields[subCategoryIndex][subCategoryName];
    ...
    if (subCategoryIndex == 0) {
        if (fields.length != 0) {
            fields.forEach((item, index) => {
                const div = document.createElement("div");
                div.setAttribute("data-index", index);
                div.setAttribute("style", "display: flex; flex-direction: row; justify-content: space-between; align-items: center");
                div.id = index;
                div.className = "divOptions";
                if (activeBalanceSheetFields[index] == 0) {
                    div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}" onchange="updateActiveFundaFields(${subCategoryIndex}, ${index})">
                `;
                } else {
                    div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}" onchange="updateActiveFundaFields(${subCategoryIndex}, ${index})" checked>
                `;
                }
                selectElement.appendChild(div);
            });
        ...
        if (fields.length != 0) {
            fields.forEach((item, index) => {
                ...
                div.id = index;
                div.className = "divOptions";
                if (activeCashFlowFields[index] == 0) {
                    div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}" onchange="updateActiveFundaFields(${subCategoryIndex}, ${index})">
                `;
        ...
        if (fields.length != 0) {
            fields.forEach((item, index) => {
                ...
                div.id = index;
                div.className = "divOptions";
                if (activeIncomeStatementFields[index] == 0) {
                    div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}" onchange="updateActiveFundaFields(${subCategoryIndex}, ${index})">
                `;
        ...
        if (fields.length != 0) {
            fields.forEach((item, index) => {
                ...
                div.id = index;
                div.className = "divOptions";
                if (activeQuarterlyIncomeFields[index] == 0) {
                    div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}" onchange="updateActiveFundaFields(${subCategoryIndex}, ${index})">
                `;
    ...
    function search() {
        const searchQuery = this.value;
        filterOptionsBySearch(searchQuery, fields, "fundaIndicators");
    }
    ...
    function filterbyLetter(subCategoryIndex) {
        const letter = this.getAttribute('data-letter');

        if (letter === '*') {
            if (subCategoryIndex == 1)
                resetFilter("cashFlow");
            else if (subCategoryIndex == 2)
                resetFilter("incomeStatement");
            else if (subCategoryIndex == 3)
                resetFilter("quarterlyIncome");
            else
                resetFilter("balanceSheet");
        } else {
            if (subCategoryIndex == 1)
                filterOptionsByLetter(letter, fields, "cashFlow");
            else if (subCategoryIndex == 2)
                filterOptionsByLetter(letter, fields, "incomeStatement");
            else if (subCategoryIndex == 3)
                filterOptionsByLetter(letter, fields, "quarterlyIncome");
            else
                filterOptionsByLetter(letter, fields, "balanceSheet");
...
function updateActiveFundaFields(subCategoryIndex, fieldIndex) {
    if (subCategoryIndex == 0) {
        activeBalanceSheetFields[fieldIndex] = activeBalanceSheetFields[fieldIndex] === 0 ? 1 : 0;
    } else if (subCategoryIndex == 1) {
        activeCashFlowFields[fieldIndex] = activeBalanceSheetFields[fieldIndex] === 0 ? 1 : 0;
    } else if (subCategoryIndex == 2) {
        activeIncomeStatementFields[fieldIndex] = activeBalanceSheetFields[fieldIndex] === 0 ? 1 : 0;
    } else {
        activeQuarterlyIncomeFields[fieldIndex] = activeBalanceSheetFields[fieldIndex] === 0 ? 1 : 0;
    }
}
    ...
    list.forEach((item, index) => {
        if (item.name.toLowerCase().includes(query.toLowerCase())) {
            ...
            `;
                ...
                div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}">
                `;
...
function filterOptionsByLetter(letter, list, listName) {
    ...
    list.forEach((item, index) => {
        if (item.name.toLowerCase().startsWith(letter.toLowerCase())) {
            ...
            <button class="indiOptions" onclick="selectOverlayIndicator(event, ${index});" style="background-color: white;">${item.name}</button>
            `;
            } else if (listName.localeCompare("cashFlow") == 0) {
                div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}">            
                `;
            } else if (listName.localeCompare("incomeStatement") == 0) {
                div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}">            
                `;
            } else if (listName.localeCompare("quarterlyIncome") == 0) {
                div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}">            
                `;
            } else if (listName.localeCompare("balanceSheet") == 0) {
                div.innerHTML = `
                    <label for="field${index}" style="width: 100%">${item.name}</label><br>
                    <input type="checkbox" id="field${index}" name="field${index}" value="${item.name}">            
                `;
...
searchSelectIndicator.addEventListener("mousedown", (e) => {
    if (!document.getElementById("indiSettingsContainer").contains(e.target) && !searchIndicatorInput.contains(e.target) && !indiCategories.contains(e.target) && !subCategoriesWrapper.contains(e.target))
