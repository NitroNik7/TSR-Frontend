...
var csh = (function () {
	...
	function getLeftColumn() {
		...
		for (let i = 0; i < mtgv.cs.tabs.length; i++) {
			...
			html += `	<button type="button" class="btn btn-light dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false" onclick="myTsrScreener.showControl('${mtgv.cs.tabs[i]["id"]}')">`
			html += mtgv.cs.tabs[i]["label"];
			html += `	</button>`
			html += `	<ul id="${mtgv.cs.tabs[i]["id"] + dropdownIdSuffix}" class="dropdown-menu" style="max-height: 500px; overflow-y: auto;">`
			let subMenu = mtgv.cs.tabs[i]["subMenu"];
			if (jsu.isNotNull(subMenu)) {
				let func = mtgv.cs.tabs[i]["func"];
				for (let j = 0; j < subMenu.length; j++) {
					let filter = subMenu[j];
					html += `<li>
								<a class="dropdown-item text-black fs-6" href="#" onclick="${func}('${filter['type']}')">${filter["label"]}</a>
							</li>`
				}
			}
