...
var csma = (function () {
	...
	*/
	...
	// FAKE_BO_BD [0].id, priceField : FAKE_PRICE_FIELD[0]
	...
	// var MA_TYPE = mtgv.mtpp.MA_TYPE;
	// var MA_PRICE_OPTIONS = mtgv.mtpp.MA_PRICE_OPTIONS;
	...
	function getMaHtml(id) {
		...
		html = '<br/>'
		html += '<div id="' + id + 'Div" class="container">';
	...
	// 	html += SP_3 + getButtonP("Compare Two MA's ", 'csma.addMa', 'maco');
		...
		// html+= SP_3 + getButtonP('Compare Price/MA ' , thisAlias+'.'+'screenNow', 'run');
		// html+= SP_3 + getButtonP('Compare Price/MA ' , thisAlias+'.'+'screenNow', 'run');
		// html += '<br/>';
		// html += '<br/>';
		...
		/*
				...
				var  helpText = "Select <b>Add To Compare MA to Price</b> to compare any EMA/SMA (upto 200) with  with Price. "
				+" You can add as many Moving Averages till your criteria is fulfilled. For Example Price Cross Above 200 MA (Long Term trend) "+
				" and Price Above 5 Day EMA for short term Signal "
		
				+"<br/> Select <b>Add To Compare MA Cross-over</b> In this option you can compare any "
				+" two Moving average for Cross Over or to find their relative position";
		
				helpText+="<br/>SMA - Simple Moving Average"
				+"<br/>EMA - Exponential Moving Average"
				+"<br/>WMA - Weighted Moving Average (Availble in higher plans)"
				+"<br/>Tick ? - Now you have flexibility to choose cross Ticks Moving Average Comparision. For Ex.  Abv Daily 50 SMA "
				+ ", Weekly 20 EMA and 30 Mins 5 WMA (Availble in higher plans)"
		
				+"<br/>Screener Tick  - Default Tick, it changes as you change Screener Tick ";
	...
	function addMa(type) { //MA_PRICE_OPTIONS
		...
		if (type == 'pma') {
			...
			$('#maCtrlTab').append(getPmaHtml(pmaObj));
			pmaChg(id);
		}
		if (type == 'maco') {
			...
			$('#maCtrlTab').append(getMacoHtml(macoComp));
			macoChg(id);
		}
		...
		if (type == 'maTrend') {
			...
			$('#maCtrlTab').append(getMaTrendHtml(maTrendComp));
			maTrendChg(id);
		}

		if (type == 'maFakeBreak') {
			var maFakeBreakComp = {
				id: id, ops: FAKE_BO_BD[0].id, maType: mtgv.mtpp.MA_TYPE[0].id,
				priceField: FAKE_PRICE_FIELD[0].id, fakePeriod: 1, maxfakePc: .75, period: 10
			}

			scrData.maFakeBreakComp.push(maFakeBreakComp);
			$('#maCtrlTab').append(getMaFakeBreakHtml(maFakeBreakComp));
			maFakeBreakChg(id);
		}

		if (type == 'maSupResBounce') {
			...
			$('#maCtrlTab').append(getMaSupResBounceHtml(maSupResBounceComp));
		...
		if (type == 'maCon') {
			var maConComp = {
				id: id, maType1: mtgv.mtpp.MA_TYPE[0].id, maType2: mtgv.mtpp.MA_TYPE[0].id,
				ma1Trend: MA_TREND[0].id, ma2Trend: MA_TREND[0].id, conDays: 10
			}
			scrData.maConComp.push(maConComp);
			$('#maCtrlTab').append(getMaConHtml(maConComp));
		...
		if (type == 'maDiv') {
			...
			$('#maCtrlTab').append(getMaDivHtml(maDivComp));
			maDivChg(id);
		}
	...
	**************************************************************************************/
	...
	function getCustScrFilter(filer, defFilter) {
		...
		filer.push({
			id: "pmaComp", label: 'Price MA Comparision', slabel: 'Moving Average', tab: MA_CS,
			type: 'btn', filtDef: { obj: thisObject, fnc: 'addMa', params: 'pma' }, subDef: mtgv.mtpp.MA_TYPE
		});

		filer.push({
			id: "macoComp", label: 'MA Cross Over', slabel: 'Moving Average Crossover', tab: MA_CS,
			type: 'btn', filtDef: { obj: thisObject, fnc: 'addMa', params: 'maco' }, subDef: mtgv.mtpp.MA_TYPE
		});


		filer.push({
			id: "maTrendComp", label: 'MA Trend', slabel: 'Trending Moving Average', tab: MA_CS,
		...
		if (mtgv.mtpp.crossFreq) {
			...
			filer.push({
				id: "maFakeBreakComp", label: 'MA Fake Breakout', slabel: 'Moving Average Fake Breakout', tab: MA_CS,
				type: 'btn', filtDef: { obj: thisObject, fnc: 'addMa', params: 'maFakeBreak' }, subDef: mtgv.mtpp.MA_TYPE
			});


			filer.push({
				id: "maSupResBounceComp", label: 'Bounce From MA', slabel: 'Bounce from Moving Average', tab: MA_CS,
				type: 'btn', filtDef: { obj: thisObject, fnc: 'addMa', params: 'maFakeBreak' }, subDef: mtgv.mtpp.MA_TYPE
			});

			filer.push({
				id: "maConComp", label: 'MA Convergence', slabel: 'Moving Average Convergence', tab: MA_CS,
				type: 'btn', filtDef: { obj: thisObject, fnc: 'addMa', params: 'maCon' }, subDef: mtgv.mtpp.MA_TYPE
			});

			filer.push({
				id: "maDivComp", label: 'MA Divergence', slabel: 'Moving Average Divergence', tab: MA_CS,
