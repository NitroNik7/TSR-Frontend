...
var htmlU = mintHtmlUtil;
	...
	function csAebbChg(csTypeId){   // AEBB Above Equals, Below, Between 
		...
		if(ops == null && mtgv.cs.ng){
			ops = CS_ABOVE;
		}


		var tds = csh.aebbHtm( csTypeId,ops,v1,v2);
		addMsgToDiv(''+csTypeId+'Td2Div', true, tds.ctrl);
...
*/
	...
	function validateFields(runType){
		...
		mtgv.cs.validResults = validResults;
		...
		let totalFieldCountHtml = validResults.validFieldCount + validResults.invalidFieldCount;
		totalFieldCountHtml += `<span class="visually-hidden">unread messages</span>`
        

		htmlU.addMsgToDiv( 'tsrMoCsSelCount' , true , totalFieldCountHtml); 
	...
	function doOpsCompValidate(def, validResults){
		...
		for(var i=0;i<arr.length;i++){
			...
			if(obj.hasData ){
				...
				if(obj.goodData){
					var ops = getObjFrmArr( BASIC_OPS,  obj.ops); 
					text  =    htmlU.doBold(def.label ) + ' :  "'+field.label +  '" ' + ops.label +' ' + obj.v1 ; 
					if(ops.id == CS_BETWEEN) text+= ' And ' +obj.v2;
					text+= ' %'
				...
				if(jsu.isNull( obj.csType) ){
					obj.csType = def.csType;
				}
	...
	function editRow(tab, obj, state){
		...
		if(	json.type =='aebb'){
			...
			html = window[tabObj.tabObj]['gftd'](json.id , null, state);
			...
			html = window[tabObj.tabObj]['gftd'](json.type, json.id, state);
		...
		mtgv.cs.editActive.push({id: json.id}); 
		if(isMobile() ){
		...
		let divEle = document.getElementById(json.id);

		if(jsu.isNull(state)){
			divEle.classList.remove("tsrCsNgDisabledFilter");
			
		}else { // (jsu.isNotNull(state))
...
*/	
		}else if(json.type == 'hlSustain'){  // Individual Object ...
			let param = "hlCs_hls"
			...
			let susObj = json.scrData.hlSustain;
			susObj.hlSusHist =NA_VAL;
			
			$("#" + 'hlSusHistDd').val(NA_VAL).change();
			susObj.hasData = false;
			susObj.goodData = false;
...
*/
			...
			// alert('todo');	
			let param =  PRICE_CS + "_" +  "gainLoss" // from dataDef
			setObjDelCommonFnc( json.scrData[json.type],json.id,  json.type + 'Ops' ,'ops', param); // priceGainLoss
...
*/
		...
		// 	$('#'+json.type  ).prop('checked', false); 
			...
			var rowId =  json.type ; //+ 'Td2Div';
			$("#" + rowId).remove();
			json.scrData[json.type].enabled= false;
			$('#'+json.type +'CB' ).prop('checked', false); 
	...
	function getSelectedField(obj){
		...
		if(type == 'aebb' || type =='trend' || type =='hlSus' 
			|| jsu.containsString( ['priceGainLoss' , OPEN_RANGE_OLD , GAP_FILL ,GAP_RUNAWAY 
			 ,TREND_CANDLE_BOBD ,  PREV_RANGE_BOBD, OPEN_RANGE_NG , GAPS_NG] , type )

			){

			if(selObj.id == null) selObj.id = type; // 
	...
	function getItemArray(type){
		...
		if(type =='hlSustain') arr = scrData.hlSustain;
