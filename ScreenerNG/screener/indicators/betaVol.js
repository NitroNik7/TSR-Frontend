...
var csbv =  (function () {			
	...
	function getFormTd(type, id, state){ //MA_PRICE_OPTIONS
		...
		if(type == 'prc'){
			return  csh.opsCompTd(obj, def).td2;
		}else if( type == 'beta'){
			return  csh.opsCompTd(obj, def).td2;
		}
	...
	function getCustScrFilter(filer, defFilter){
		if(!jsu.isMigContext()){
			...
			filer.push({  id :  "prComp" , label : 'Price Range'    , tab : BV_CS, 
				type : 'btn'  , filtDef : {obj:'csu', fnc: 'opsCompare' , params:  'prc' } , subDef :AVG_PRICE_RANGE, mobFilter: "betaCs_priceRange" }) ;  //   JavaScript:cscmn.atn('price','priceCs');
			//onclick="JavaScript:csu.opsCompare('beta');"

			filer.push({  id :  "betaComp" , label : 'Beta '  ,  tab : BV_CS, 
				type : 'btn'  , filtDef : {obj:'csu', fnc: 'opsCompare' , params:  'beta' } , subDef : BETA_PERIOD , mobFilter: "betaCs_Beta"}) ;  //   JavaScript:cscmn.atn('price','priceCs');

		}

		if (!jsu.isMigContext()) {

					defFilter.push({  id :  "prComp" , label : 'Price Range'    , tab : BV_CS,   mobFilter: "betaCs_priceRange",
