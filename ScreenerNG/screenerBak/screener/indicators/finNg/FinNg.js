var csFrNg = (function () {
	...
	function getHtml(id) {
		...
		if (isNg) { // added
			...
			for (var i = 0; i < NG_RATIO_LIST.length; i++) {
				...
				// createScrPortlet(divId, col , title, elems, id, type, srcObj )

			}
			/*
					html+= createScrPortlet(GURU_LIST_CODE, BOOT_4_12COL, 'Guru Numbers', GURU_NUMBERS ,  null, GURU_LIST_CODE);
			
					// Val ratio ....
			
					html+= createScrPortlet(VAL_RATIO_LIST_CODE, BOOT_4_12COL, 'Valuation Ratio', RATIO_VAL_DEF ,  null, VAL_RATIO_LIST_CODE);
			
					html+= createScrPortlet(PFT_RATIO_LIST_CODE, BOOT_4_12COL, 'Profitability Ratio', RATIO_PFT_DEF ,  null, PFT_RATIO_LIST_CODE);
					
					html+= createScrPortlet(SOL_RATIO_LIST_CODE, BOOT_4_12COL, 'Solvency Ratio', RATIO_SOL_DEF ,  null, SOL_RATIO_LIST_CODE);
					
					html+= createScrPortlet(EFF_RATIO_LIST_CODE, BOOT_4_12COL, 'Efficiency Ratio', RATIO_EFF_DEF ,  null, EFF_RATIO_LIST_CODE);
	...
	*/
	/*
		...
		function createScrPortlet(divId, col , title, elems, id, type ){
				...
				html += portletTable(elems, type, id);
		  ...
		  function portletTable(elems, type, id){
				...
				var html =''
				 html += '<table class="table table-striped table-bordered table-hover "  style="white-space: nowrap;overflow-x: scroll;">';
					...
					for(var i=0;i< elems.length ;i++){
						...
						html +=  htmlU.getButtonP('Add' ,thisObject+'.ae' ,  params  , ' Add '+elem.label );
	...
	*/
	...
	function addElem(type, baseField) {
		...
		$('#finCtrlTab').append(html);



		userAction(id);
		csu.dsf();
		// htmlU.focusToDiv(finObj.id+'Td2Div');
		// htmlU.focusToDiv('finRatNgDiv')
		// $('#finRatNgDiv').scrollTop(0);

		// $("#finRatNgDiv").animate({scrollTop: $("#finRatNgDiv").offset().top});
	...
	function addFilter(filer, ngRatio, defFilter) {
		...
		for (var i = 0; i < ratioList.length; i++) {
			...
			let obj = {
				id: "finNgComp", label: thisRatio.label + ", " + thisRatio.sLabel, slabel: thisRatio.sLabel,
