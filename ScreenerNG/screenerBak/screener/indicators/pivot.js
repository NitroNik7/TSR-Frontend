...
var cspp = (function () {
	...
	function getPpHtml(id) {
		...
		if (isNg) {
			...
			html += SP_3 + getButtonP('Price / Pivot Levels', 'cspp.addPP', 'pp');

			html += SP_3 + getButtonP('Central Pivot Levels', 'cspp.addCpr', 'cpr');

			if (mtgv.mtpp.crossFreq && mtgv.mtpp.pr) {
				html += SP_3 + getButtonP('Fibonacci Retracement', 'cspp.addFibr', 'fibr');
			} else {
	...
	function addPP(type) {
		...
		$('#ppCtrlTab').append(getPpLevelsHtml(ppObj));

		ppChg(id);
		// var html = '<tr id='+ppObj.id+'>'
		// 		 + createTd( 'Close'  , CS_LABEL_WIDTH) 
		// 		 + createTd(createDiv(tecObj.id+'Td2Div', tds.td2, null)) +'</tr>';
	...
	function addCpr(type) {
		...
		$('#ppCtrlTab').append(getCprHtml(cprObj));
	...
	function addFibr(type) {

		var id = myTsrScreener.getNextId(type + 'Id');

		var fibrObj = {
			id: id, ops: 'tknSupNvrBreach', retType: RETRACE_TYPE[0].id,
		...
		$('#ppCtrlTab').append(getFibrHtml(fibrObj));
	...
	function getCustScrFilter(filer, defFilter) {
		...
		filer.push({
			id: "ppComp", label: 'Pivot Point', tab: PP_CS,
			type: 'btn', filtDef: { obj: thisObject, fnc: 'addPP', params: 'pp' }, subDef: pivot_fields
		});  //   JavaScript:cscmn.atn('price','priceCs');


		filer.push({
			id: "cprComp", label: 'Central Pivot Point', tab: PP_CS,
			type: 'btn', filtDef: { obj: thisObject, fnc: 'addCpr', params: 'cpr' }, subDef: CPR_CS_RPT
		});  //   JavaScript:cscmn.atn('price','priceCs');

		if (mtgv.mtpp.crossFreq) {
			let obj = {
				id: "fibrComp", label: 'Fibonacci Retracement', tab: PP_CS,
