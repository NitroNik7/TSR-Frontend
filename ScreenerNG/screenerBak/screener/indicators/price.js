	...
	function getAllRows(){
		...
		if(mtgv.cs.ng){
			...
			if(csPriceObj.hasData){
				csh.aebbStrut('csPrice', null);	
			}

			let priceGainLossObj = mtgv.cs.screenerData.priceGainLoss;

			if(priceGainLossObj.gainLossType != 'na'){
				getPriceGainLossRow();	
			}
			...
			html+=     '<tr><td><b>Gain Loss in %</b></td> ' +   createTd( createDiv('priceGainLossTd2Div', createTableData(PRICE_GAIN_LOSS) ) ) + '</tr>';
