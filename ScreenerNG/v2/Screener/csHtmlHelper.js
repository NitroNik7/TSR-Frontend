...
var csh = (function () {
	...
	function getCsWrapperNg() {
		...
		csDiv.style.minHeight = "500px";
		csDiv.style.overflow = "hidden";

		let html = "";

		html += `	<div class="d-flex flex-column border p-2"  style="min-height: 500px; height: 70%;">` //  style="height: 70%; min-height: 300px"
		html += getTopRowNg();
		html += `		<hr>`
		html += getMiddleRowNg(); 
		html += `		<hr>`
	...
	function getMiddleRowNg() {
			...
			if (jsu.isNotNull(subMenu)) {
				...
				for (let j = 0; j < subMenu.length; j++) { // for each sub cat.
					...
					if (jsu.isNull(nestedMenu)) { // if nestedMenu does not exist, i.e. item is a single filter
						...
						html += getFilterHtml(tabId, item, idStr);
						...
						for (let k = 0; k < nestedMenu.length; k++) { // for innermost nested filters
							...
							html += getFilterHtml(tabId, nestedItem, idStr);
		...
		html += `							<p>`
		html += `								Show Selected fields using`
		html += `							</p>`
		html += `							<div class="mx-2">`
		html += `								<input type="radio" id="html" name="tsrCsSelFields" value="form">`
		html += `								<label for="html">Form</label>`
		html += `							</div>`
		html += `							<div class="mx-2">`
		html += `								<input type="radio" id="css" name="tsrCsSelFields" value="labels">`
	...
	function getFilterHtml(tabId, filterItem, idStr) {
		...
		let prUser = false;
		// if (mtgv.mtpp.crossFreq && mtgv.mtpp.pr) { // check condition
		// 	prUser = true;
		// }
		...
		if (prFilter) {
			if (prUser) {
				if (addOnce) {
					html += `<li>`
					html += `	<a id="${id}" class="dropdown-item" onclick=" csh.ttcm(); csh.cd('${tabId}'); csh.uht('${idStr}'); ${func}(${params}); csh.df(this, '${idStr}');" style="text-wrap: nowrap; font-size: 16px; color: black;">`
					html += filterItem["label"]
					html += `	</a>`
					html += `</li>`
				} else {
					html += `<li>`
					html += `	<a  id="${id}"class="dropdown-item" onclick=" csh.ttcm(); csh.cd('${tabId}'); csh.uht('${idStr}'); ${func}(${params})" style="text-wrap: nowrap; font-size: 16px; color: black;">`
					html += filterItem["label"]
					html += `	</a>`
					html += `</li>`
				}
			} else { // show filter with PRO badge
				html += `<a id="${id}" class="dropdown-item d-flex justify-content-between align-items-baseline" href="${plansUrl}" style="text-wrap: nowrap; font-weight: 300; font-size: 16px; color: black;">`
				html += filterItem["label"]
				html += `	<span class="badge bg-warning ms-2">PRO</span>` // todo PRO / UPGRADE badge
				html += `</a>`
			}
		} else {
			if (addOnce) {
	...
	function disableFilter(filterHtmlEle, idStr) {

		let filterObj = getFilterObjFromScrData(idStr);

		filterHtmlEle.onclick = null;

		filterHtmlEle.setAttribute("style", "text-wrap: nowrap; font-size: 16px; color: grey;");
		filterHtmlEle.classList.toggle("tsrCsFilter"); // .tsrCsFilter css in style.css

		let html = ``;

		html += filterObj["label"]
		html += `		<span style="font-size: 12px; text-align: end;">`
		html += `			Already added `
		html += `		</span>`
		// html += `	</a>`

		filterHtmlEle.innerHTML = html;
	}

	function updateHelpText(idStr) { // idStr
			...
			html += `
					<div class="dropend">
						...
						<div class="dropdown-menu text-center p-3">
							<div>
								<img src="https://tutorials.topstockresearch.com/ChartPatterns/Triangles/TypesofTrianglesChartPatterns.png" class="img-fluid img-thumbnail" alt="...">
							</div>
							<div>
								 <p>
	...
	// 	html += `		<div class="d-flex">` // todo for laptops: max-height: 300px;overflow: auto;
	...
	// 					func = `myTsrScreener.showControl('${mtgv.cs.tabs[i]["id"]}');`;
	// 					if (isNotNull(params) && params != "") {
	// 						func += item["func"] + "(" + params + ");";
	// 					} else {
	// 						func += item["func"] + "();";
	// 					}
	...
	// 	let onclick = `
	// 					myTsrScreener.showControl('${mtgv.cs.tabs[i]['id']}'); 
	// 					let cont = document.getElementById('${mtgv.cs.tabs[i]['tab']}'); 
	// 					cont.innerHTML = ${item['func'] + '(' + params + ')'};
	// 				`

	// 	listHtml += `	<a class="dropdown-item text-black fs-6 d-flex justify-content-between align-items-baseline" href="#" onclick="${onclick}
	...
	// 	listHtml += `</li>`
	// } else {
	// if (jsu.isNotNull(item["cb"]) && item["cb"]) { // item is checkbox
	// 	listHtml += `
	// 				<li class="dropdown-item text-black fs-6 ps-3 d-flex justify-content-between align-items-baseline">
	// 					<label for="${item["id"]}">${item["label"]}</label>
	// 					<input type="checkbox" class="ms-2" id="${item["id"]}" onclick="${func}">
	// 				</li>
	// 			`
	// } else {
	...
	// 					if (item["premium"]) {
	// 						if (mtgv.mtpp.pr) { // check if mtgv.mtpp.crosFreq is true for every premium user ?
	// 							if (mtgv.mtpp.crossFreq) {
	// 								// show filter
	// 								if (item["many"]) {

	// 								} else {
	// 									// disable once enabled. It should be possible to remove it only from sel. fields
	// 									// onclick:
	// 									// 		disable in menu
	// 									// replace html entirely ?
	// 									// toggle classes/onclick ?
	// 									// show message when item is already selected - filter already applied. Can be applied only once - use tooltip / popover if possible
	// 									// removing from sel. fields - should re-enable the button
	// 								}
	// 							} else {
	// 								// disable filter - add badge - PRO / UPGRADE, link to plan page
	// 							}
	// 						} else {
	// 							// disable filter - add badge - PRO / UPGRADE, link to plan page
	// 						}
	...
	// 					// if (mtgv.mtpp.crossFreq && mtgv.mtpp.pr && item["premium"]) { // if item is only for premium
	// 					// 	listHtml += `	<a disabled class="dropdown-item text-black fw-lighter fs-6 d-flex justify-content-between align-items-baseline" href="#"  style="text-wrap: nowrap;">`
	// 					// 	listHtml += item["label"]
	// 					// 	listHtml += `	<span class="badge bg-warning ms-2">PRO</span>`
	// 					// } else {
	// 					listHtml += `	<a class="dropdown-item text-black fs-6 d-flex justify-content-between align-items-baseline" href="#" onclick="${func}" style="text-wrap: nowrap;">`
	// 					listHtml += item["label"]
	// 					// }
	// 					listHtml += `	</a>`
	// 					listHtml += `</li>`
	...
	// 						func = `myTsrScreener.showControl('${mtgv.cs.tabs[i]["id"]}');`;
	// 						if (isNotNull(params) && params != "") {
	// 							func += nestedItem["func"] + "(" + params + ");";
	// 						} else {
	// 							func += nestedItem["func"] + "();";
	// 						}

	// 						// if (jsu.isNotNull(nestedItem["cb"]) && nestedItem["cb"]) {
	// 						// 	html += `
	// 						// 			<li class="dropdown-item text-black fs-6 ps-3 d-flex justify-content-between align-items-baseline">
	// 						// 				<label for="${nestedItem["id"]}">${nestedItem["label"]}</label>
	// 						// 				<input type="checkbox" class="ms-2"  id="${nestedItem["id"]}" onclick="csp.tcc();">
	// 						// 			</li>
	// 						// 	`
	// 						// } else {
	// 						html += `<li>`

	// 						if (mtgv.mtpp.crossFreq && mtgv.mtpp.pr && nestedItem["premium"]) { // if item is only for premium
	// 							html += `	<a disabled class="dropdown-item text-black fw-lighter fs-6 d-flex justify-content-between align-items-baseline" href="#"  style="text-wrap: nowrap;">`
	// 							html += nestedItem["label"]
	// 							html += `	<span class="badge bg-warning ms-2">PRO</span>`
	// 						} else {
	// 							html += `	<a class="dropdown-item text-black fs-6 d-flex justify-content-between align-items-baseline" href="#" onclick="${func}" style="text-wrap: nowrap;">`
	// 							html += nestedItem["label"]
	// 						}
	// 						html += `	</a>`
	// 						html += `</li>`
	...
	function getFilterControlNg() { // todo use functions in place of button html
		...
		html += htmlU.getCusBtn('Run ', func, 'run', 'Run Screener', custBtnClass, 'fas fa-play', tabindex); // todo add style to btn
	...
	function printHtml(tabId, params) {
		let tab = mtgv.cs.tabs[tabId];
		myTsrScreener.showControl(tab["id"]);
		// 		html += csh.aebbStrut(PRICE_AEBB_MAP[i].id, PRICE_LABEL_WIDTH);
		let arr = params.split(",")
		getAebbStructureHtmlNg(tab["tab"], arr[0], arr[1]);
	}
