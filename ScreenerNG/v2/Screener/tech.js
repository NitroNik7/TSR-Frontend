...
var cst = (function () {
	...
	function getTiHtml(id) {
		...
		if (isNg) { // added
			...
			/*			
					if(jsu.isMigContext()){
						aioRsiChange();  // Mess  to RESOLVE later 
						...
						html+= SP_3 + htmlU.getButtonP('RSI' , 'cst.atn', RSI + PARAM_DELIM + 'obos','RSI (Relative Strength Indicator)');
						html+= SP_3 + htmlU.getButtonP('RSI(S)' , 'cst.atn', RSI_SMOOTH+ PARAM_DELIM  + 'obos','Smooth RSI ( Relative Strength Indicator)');
			...
			*/
			...
			/*		
					...
					var  helpText ='';
					helpText+= BR_2  + doBold( 'Major Upgrade in Progress' ) + ' Many more indicators coming soon' ;
			
					// helpText+= BR_2 + doBold( 'Custom Tech') + ' Custom Tech like MFI 14 ... is work in progress. Expected to be completed by end of Dec' ;
			
					// helpText+= BR_2  + doBold( 'Divergence Coming Soon');
			
			
			
					 helpText += BR_2 + "Select <b>Compare Indi with Abs Value</b> To Compare Any Indicator Value in a Range. Fo Ex ADX > 25 . "
					+" You can add as Indicators as you want."
			
					+"<br/><br/> Select <b>Compare Tech Indi Crossovers</b> To filter recent Cross overs of technical Indicators. For Ex Macd Cross Above Signal Line or RSI Rising above 30"
			
					+"<br/><br/> Select <b>Ichimoku Cloud Filter</b> Similar to Above other two Options this option also Provides you with Various options in Ichimoku. Dure to vastness, we have kept it separate"
					+"<br/> <b>Ichimoku Indicator/ Ichimoku Kinko Hyo Terms</b>"
					+"<br/> <b>Ichimoke Cloud</b> - IC - Kumo  - "
					+"<br/> <b>Leading Span A </b> - LSA - Senkou Span A "
					+"<br/> <b>Leading Span B </b> - LSB - Senkou Span B "
					+"<br/> <b>Base Line </b> - BL - Kijun Line "
					+"<br/> <b>Conversion Line </b> - CL -Tenkan-Sen  - kenkan sen"
					+"<br/> <b>Lagging Span </b> - LAG -Chikou Span"
	...
	function getMacoTd(techObj, objDef, func) {
		...
		html += SP_3 + getDropDown(mtgv.mtpp.MA_TYPE, id + 'maType', null, func, id, techObj.maType);
		...
		html += SP_3 + getInputTxtParam(id + 'v1', 3, techObj.v1, func, id);
		html += SP_3 + htmlU.getSpan('(Supported value 2 to 20) ', 'grey', 8)
	...
	function getCustScrFilter(filer, defFilter) {
		...
		for (var i = 0; i < ALL_INDIS_MAP.length; i++) {
			...
			let obj = {
				id: "techNgComp", label: thisIndi.label, slabel: thisIndi.shortName, tab: TI_CS,
		...
		filer.push({
			id: "techNgComp", label: 'Bollinger Band Squeeze', tab: TI_CS,
