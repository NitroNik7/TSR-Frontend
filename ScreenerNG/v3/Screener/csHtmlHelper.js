...
var csh = (function () {
	...
	function getMiddleRowNg() {
		...
		html += `					<div id="tsrCsSelFieldsCheckRow" style="display: block;">`
		html += getSelFieldsCheckRowHtml();
		html += `					</div>`
		html += `					<div id="tsrCsInstrBox" style="display: none; max-width: 30%;">`
	...
	function getSelFieldsCheckRowHtml() {
		...
		html += `						<div class="mx-3">`
		// html += `					<div class="mx-3 d-flex justify-content-between">`
		...
		html += `								</div>`
		html += `								<div class="mx-2">`
		html += `									<input type="checkbox" id="css" value="sfLabel" onchange="csh.usf();">`
...
// todo integrate
	...
	function showInstrBox(show) { // todo - this function should be called after reset / if atleast one filter is selected
	...
	function getFilterHtml(tabId, filterItem, idStr) {
		...
		let subscribedUser = false; // todo which filters are available for premium / other plan users ?
		let prUser = false;
		if (mtgv.mtpp.crossFreq) {
			if (mtgv.mtpp.pr) {
				prUser = true;
