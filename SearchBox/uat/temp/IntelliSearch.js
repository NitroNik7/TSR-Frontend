var miIs = (function () {  // chart init Params
    ...
    function init(mobile) {
        ...
        if (mobile) {
            ...
            onclick="miIs.ssb('equity')" autocomplete="off" style="width:95%; margin:10px">`;
        } else {
    ...
    function populateSearchList(searchMenuData, stockList, buttons, base_url) {
        ...
        stockList.forEach(element => {
            ...
            if (searchMenuData.jsFnc) { // set onclick handler
                ...
                if (buttons.length == 0) {
                    li = createOnClick(li, searchMenuData, element);
                }
                else {
                    li = createOnClickAndButtons(li, element, buttons, base_url, searchMenuData);
                ...
                li = createLinkAndButtons(li, element, buttons, base_url);
            }
    ...
    function createLinkAndButtons(li, element, buttons, base_url) {
        ...
        for (let i = 0; i < buttons.length; i++) {
            ...
            if (paramDefined(buttons[i].jsFnc) && buttons[i].jsFnc) {
                ...
                button.innerHTML = `
                            <a href="${base_url + buttons[i].urlPrefix + "/" + element.code + "/" + buttons[i].id}" style="color: black; padding: 10px;">
                            ${buttons[i].label}
                            </a>`;
            }
