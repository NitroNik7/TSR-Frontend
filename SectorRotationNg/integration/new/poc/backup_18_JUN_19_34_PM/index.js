...
var miSrn = (function () {  // chart init Params
    ...
    function init() {
        ...
        if (validateSecRotSettings()) {
            ...
            if (jsu.isNotNull(mintJsUtil.getRootUrl())) {
                url = mintJsUtil.getRootUrl() + `/djs?id=${duration}&type=${sectorType}&cat=SecRot&action=all`; // TODO - use this later
            } else {
    ...
    function paintSectorTableSection() {
        ...
        if (isIdxBased) {
            html += `               <div class="bg-black bg-opacity-10 border-top mt-4 p-3 d-flex flex-column flex-sm-row align-items-center justify-content-end" style="gap: 10px;">                  
                                        <p style="margin-bottom: 0; text-transform: uppercase;letter-spacing: 1px;font-size: 13px;font-weight: bold;color: #64748b;">
                                            View Sectors on Chart
                                        </p>                   
                                        <div class="d-flex" style="gap: 10px;">                    
                                            <a class="tsrSecRotTabBtnIdx" style="cursor:pointer; padding: 5px 16px;" onclick="miSrn.pc('op', 0, 'sectorList', '', true, 'inline', true);" oncontextmenu="return false;"> 
                                                <span class="fas fa-chart-line"></span> Inline 
                                            </a>         
                                            <a class="tsrSecRotTabBtnIdx" style="cursor:pointer;  padding: 5px 16px;" onclick="miSrn.pc('op', 0, 'sectorList', '', true, 'tile', true);" oncontextmenu="return false;">
        ...
        if (isIdxBased) {
            html += `               <div class="bg-black bg-opacity-10 border-top mt-4 p-3 d-flex flex-column flex-sm-row align-items-center justify-content-end" style="gap: 10px;">                  
                                        ...
                                        <div class="d-flex" style="gap: 10px;">                    
                                            <a class="tsrSecRotTabBtnIdx" style="cursor:pointer; padding: 5px 16px;" onclick="miSrn.pc('up', 0, 'sectorList', '', true, 'inline', true);" oncontextmenu="return false;"> 
                                                <span class="fas fa-chart-line"></span> Inline 
                                            </a>         
                                            <a class="tsrSecRotTabBtnIdx" style="cursor:pointer;  padding: 5px 16px;" onclick="miSrn.pc('up', 0, 'sectorList', '', true, 'tile', true);" oncontextmenu="return false;">
                                                <span class="fas fa-chart-line"></span> Tile  
                                            </a>       
                                        </div>   
                                    </div>`
    ...
    function updateSectorTable(tableContainerId, secType) {
        ...
        if (isIdxBased) {
            ...
            html += `           <th colspan="10">Sector Summary <span style="font-size: 10px;">(Technicals)<span></th>`
        }
    ...
    function paintStockTableSection(sector, secType, stockType, secId) {
        ...
        if (stockList.length > 0) {
            ...
            html += `           <a class="tsrSecRotTabBtnIdx" style="cursor:pointer; padding: 5px 16px;" onclick="miSrn.pc('${secType}', ${secId}, '${stockType}List', '', true, 'inline', true);" oncontextmenu="return false;"> <span class="fas fa-chart-line"></span> Inline </a>`
            // miSrn.pss('${secType}', '${stockType}', ${secId}, '', false); miSrn.pc('${secType}', ${secId}, '${stockType}List', '', true, 'tile', true);
            html += `           <a class="tsrSecRotTabBtnIdx" style="cursor:pointer;  padding: 5px 16px;" onclick="miSrn.pc('${secType}', ${secId}, '${stockType}List', '', true, 'tile', true);" oncontextmenu="return false;">  <span class="fas fa-chart-line"></span> Tile  </a>`
            html += `       </div>`
            html += `   </div>`
            ...
            if (!(isSectorLoaded(currSectorData, secType, secId))) {
                miSrnUtils.gd(url).then(data => {
                    ...
                    if (data.statusCode == "success") {
                        ...
                        // params.stock
                        miStkHl.pss(params, false, stockHlPopupId);
