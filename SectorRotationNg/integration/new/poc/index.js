...
function paintStockSection(secType, sector, stockType, secId, stockCode, stock) {
    ...
    const signCls = v => (parseFloat(v) >= 0 ? "tsrSecRot-pos" : "tsrSecRot-neg");

    // ╔══════════════════════════════════════════════════════╗
    // ║                    HEADER                           ║
    // ╚══════════════════════════════════════════════════════╝
    let headerHtml = `
    <div class="tsrSecRot-header">
        <div class="tsrSecRot-header-inner">

            <!-- LEFT: ticker badge + name -->
            <div class="tsrSecRot-title-group">
                <span class="tsrSecRot-ticker-badge">${stock["id"]}</span>
                <div>
                    <div class="tsrSecRot-stock-name">${stock["name"]}</div>
                    <div class="tsrSecRot-header-sub">
                        <span class="tsrSecRot-price-tag">
                            <i class="fas fa-rupee-sign me-1"></i>${fmt(stock["price"])}
                        </span>
                        <span class="tsrSecRot-change-tag ${signCls(stock["priceChange"])}">
            ...
            <div class="tsrSecRot-header-actions">
                <span class="tsrSecRot-view-label">View Analysis</span>
                <a href="${techUrl}" target="_blank" class="tsrSecRot-action-btn tsrSecRot-btn-tech" oncontextmenu="return false;">
                    <i class="fas fa-chart-line me-1"></i>Technical
                </a>
                <a href="${fundaUrl}" target="_blank" class="tsrSecRot-action-btn tsrSecRot-btn-funda" oncontextmenu="return false;">
        ...
        <!-- accent rule -->
        <div class="tsrSecRot-header-rule"></div>
    </div>`;

    // ╔══════════════════════════════════════════════════════╗
    // ║                     BODY                            ║
    // ╚══════════════════════════════════════════════════════╝
    let bodyHtml = `<div class="tsrSecRot-body">`;

    /* ── 1. HIGHLIGHTS ── */
    bodyHtml += `
    <section class="tsrSecRot-section">
        <div class="tsrSecRot-section-hd">
            <span class="tsrSecRot-section-icon tsrSecRot-icon-hl"><i class="fas fa-bolt"></i></span>
            <h6 class="tsrSecRot-section-title">Highlights</h6>
        </div>
        <div class="tsrSecRot-highlights-grid">

            <div class="tsrSecRot-hl-card tsrSecRot-hl-nifty">
                <span class="tsrSecRot-hl-label">vs NIFTY</span>
                <span class="tsrSecRot-hl-value ${signCls(stock["vsNifty"])}">${fmt(stock["vsNifty"])}%</span>
                <span class="tsrSecRot-hl-icon"><i class="fas fa-chart-area"></i></span>
            </div>`;

    if (jsu.isNotNull(stock.vsIdx) && stock.vsIdx) {
        bodyHtml += `
            <div class="tsrSecRot-hl-card tsrSecRot-hl-sector">
                <span class="tsrSecRot-hl-label">vs ${sector["name"]}</span>
                <span class="tsrSecRot-hl-value ${signCls(stock["vsIdx"])}">${fmt(stock["vsIdx"])}%</span>
                <span class="tsrSecRot-hl-icon"><i class="fas fa-building"></i></span>
            </div>`;
    }

    bodyHtml += `
            <div class="tsrSecRot-hl-card tsrSecRot-hl-price">
                <span class="tsrSecRot-hl-label">Current Price</span>
                <span class="tsrSecRot-hl-value tsrSecRot-neutral">₹${fmt(stock["price"])}</span>
                <span class="tsrSecRot-hl-icon"><i class="fas fa-rupee-sign"></i></span>
            </div>

            <div class="tsrSecRot-hl-card tsrSecRot-hl-change">
                <span class="tsrSecRot-hl-label">Price Change</span>
                <span class="tsrSecRot-hl-value ${signCls(stock["priceChange"])}">${fmt(stock["priceChange"])}%</span>
                <span class="tsrSecRot-hl-icon"><i class="fas fa-percentage"></i></span>
            </div>
    ...
    <section class="tsrSecRot-section">
        <div class="tsrSecRot-section-hd">
            <span class="tsrSecRot-section-icon tsrSecRot-icon-tsr"><i class="fas fa-tachometer-alt"></i></span>
            <h6 class="tsrSecRot-section-title">TSR Metrics</h6>
        </div>
        <div class="tsrSecRot-metrics-row">`;

    tsrMetrics.forEach(m => {
        bodyHtml += `
            <div class="tsrSecRot-metric-pill">
                <span class="tsrSecRot-metric-icon" style="color:${m.clr}"><i class="fas ${m.icon}"></i></span>
                <div class="tsrSecRot-metric-text">
                    <span class="tsrSecRot-metric-label">${m.label}</span>
                    <span class="tsrSecRot-metric-val" style="color:${m.clr}">${m.val}</span>
                </div>
            </div>`;
    });
    ...
    bodyHtml += `<div class="tsrSecRot-dual-row">`;

    // Returns
    bodyHtml += `
    <section class="tsrSecRot-section tsrSecRot-half">
        <div class="tsrSecRot-section-hd">
            <span class="tsrSecRot-section-icon tsrSecRot-icon-ret"><i class="fas fa-history"></i></span>
            <h6 class="tsrSecRot-section-title">Returns</h6>
        </div>
        <div class="owl-carousel owl-theme tsrSecRot-carousel periodicReturns">`;

    stock["tsrRtn"].forEach((r, j) => {
        if (jsu.isNull(r["label"])) return;
        const v = parseFloat(r["stkRtn"]);
        bodyHtml += `
            <div class="tsrSecRot-carousel-card">
                <span class="tsrSecRot-cc-label">${r["label"]}</span>
                <span class="tsrSecRot-cc-value ${v >= 0 ? 'tsrSecRot-pos' : 'tsrSecRot-neg'}">
    ...
    <section class="tsrSecRot-section tsrSecRot-half">
        <div class="tsrSecRot-section-hd">
            <span class="tsrSecRot-section-icon tsrSecRot-icon-ema"><i class="fas fa-wave-square"></i></span>
            <h6 class="tsrSecRot-section-title">EMA</h6>
        </div>
        <div class="owl-carousel owl-theme tsrSecRot-carousel periodicReturns">`;
    ...
    stock["ema"].forEach((e, j) => {
        ...
        bodyHtml += `
            <div class="tsrSecRot-carousel-card">
                <span class="tsrSecRot-cc-label">${e["label"]}</span>
                <span class="tsrSecRot-cc-value" style="color:${e["clrl"] || '#334155'}">
                    ${fmt(e["val"])}
                </span>
                <span class="tsrSecRot-cc-sub" style="color:${e["clrl"] || '#64748b'}">${e["intr"] || ""}</span>
            </div>`;
    });
    ...
    if (jsu.isNotNull(stock["tech"])) {
        ...
        <section class="tsrSecRot-section tsrSecRot-half">
            <div class="tsrSecRot-section-hd">
                <span class="tsrSecRot-section-icon tsrSecRot-icon-tech"><i class="fas fa-microscope"></i></span>
                <h6 class="tsrSecRot-section-title">Technicals</h6>
            </div>
            <div class="owl-carousel owl-theme tsrSecRot-carousel technicals">`;

        stock["tech"].forEach((t, j) => {
            if (jsu.isNull(t["label"])) return;
            bodyHtml += `
                <div class="tsrSecRot-carousel-card tsrSecRot-cc-three">
                    <span class="tsrSecRot-cc-label">${t["label"]}</span>
                    <span class="tsrSecRot-cc-value" style="color:${t["clrl"] || '#334155'}">${miSrnUtils.gcv(t["val"], t["clrl"], null)}</span>
                    <span class="tsrSecRot-cc-sub" style="color:${t["clrl"] || '#64748b'}">${t["intr"] || ""}</span>
                </div>`;
        });
    ...
    if (jsu.isNotNull(stock["funda"])) {
        ...
        <section class="tsrSecRot-section tsrSecRot-half">
            <div class="tsrSecRot-section-hd">
                <span class="tsrSecRot-section-icon tsrSecRot-icon-fund"><i class="fas fa-landmark"></i></span>
                <h6 class="tsrSecRot-section-title">Fundamentals</h6>
            </div>
            <div class="owl-carousel owl-theme tsrSecRot-carousel technicals">`;
        ...
        stock["funda"].forEach((f, j) => {
            ...
            bodyHtml += `
                <div class="tsrSecRot-carousel-card tsrSecRot-cc-three">
                    <span class="tsrSecRot-cc-label" title="${f["label"]}">${f["label"]}</span>
                    <span class="tsrSecRot-cc-value" style="color:${f["clrl"] || '#334155'}">${miSrnUtils.gcv(f["val"], f["clrl"], null)}</span>
                    <span class="tsrSecRot-cc-sub" style="color:${f["clrl"] || '#64748b'}">${f["intr"] || ""}</span>
                </div>`;
        });
    ...
    bodyHtml += `</div>`; // dual-row
    bodyHtml += `</div>`; // tsrSecRot-body
