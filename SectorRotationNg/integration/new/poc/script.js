...
var miSrn = (function () {  // chart init Params
    ...
    function updateSectorTable(tableContainerId, secType) {
        ...
        if (isIdxBased) {
            html += `           <th colspan="6">Sector Summary <span style="font-size: 10px;">(Technicals)</span></th>`
        } else {
            html += `           <th colspan="10">Sector Summary <span style="font-size: 10px;">(Consolidated)<span></th>`
        }
        ...
        if (sectors.length == 0) {
            ...
            for (let i = 0; i < sectors.length; i++) {
                ...
                if (isIdxBased) {
                    ...
                    // TODO - do the values have to be multiplied by 100
                    ...
                    html += `           <td>${miSrnUtils.grv(tsrStr.roa)}</td>`
                    html += `           <td>${miSrnUtils.grv(tsrStr.roe)}</td>`
                    html += `           <td>${miSrnUtils.grv(tsrStr.netMgn)}</td>`
                    html += `           <td>${miSrnUtils.grv(tsrStr.ebitMgn)}</td>`
        ...
        if (isIdxBased) {
            ...
            html += `   <div class="bg-black bg-opacity-10 border-top mt-4 p-3 d-flex align-items-baseline justify-content-end" style="gap: 10px;">`
            html += `       <p style="margin-bottom: 0; text-transform: uppercase;letter-spacing: 1px;font-size: 13px;font-weight: bold;color: #64748b;">View Sectors on Chart</p>`;

            html += `       <a class="tsrHmTabBtnIdx" style="cursor:pointer; padding: 5px 16px;" onclick="miSrn.pc('${secType}', 0, 'sectorList', '', true, 'inline', true);" oncontextmenu="return false;"> <span class="fas fa-chart-line"></span> Inline </a>`
            html += `       <a class="tsrHmTabBtnIdx" style="cursor:pointer;  padding: 5px 16px;" onclick="miSrn.pc('${secType}', 0, 'sectorList', '', true, 'tile', true);" oncontextmenu="return false;">  <span class="fas fa-chart-line"></span> Tile  </a>`
            html += `   </div>`
        }
        // html += `           <div class="mt-2">`
    ...
    function paintSectorCard(secType, sectors, i) {
        ...
        // ------------------------- CARD HEADER STARTS ------------------------------------
        html += `                <div class="card-header bg-light p-4 border-bottom">`
        /*
        ...
        */
        ...
        html += `                               <a href="${sectorAnalysisUrl}" target="_blank" class="tsrHmTabBtnIdx d-none d-md-block">`
        ...
        // html += `                           <h6 class="">`;
        html += `                               <a href="${sectorAnalysisUrl}" target="_blank" class="tsrHmTabBtnIdx mx-auto" style="width: max-content;" >`
        // html += `                                   View in Depth Analysis`
        html += `                                   Sector Analysis`
        html += `                                   <i class="fas fa-external-link-square-alt"></i>`
        html += `                               </a>`
    ...
    function paintStockTableSection(sector, secType, stockType, secId) {
        ...
        if (stockList.length > 0) {
            ...
            // html += `       <p style="margin-bottom: 0; text-transform: uppercase;letter-spacing: 1px;font-size: 13px;font-weight: bold;">View Sectors on Chart</p>`;
            html += `   <div class="bg-black bg-opacity-10 border-top mt-4 p-3 d-flex align-items-baseline justify-content-end" style="gap: 10px;">`
            html += `       <p style="margin-bottom: 0; text-transform: uppercase;letter-spacing: 1px;font-size: 13px;font-weight: bold;color: #64748b;">View Stocks on Chart</p>`;

            html += `       <a class="tsrHmTabBtnIdx" style="cursor:pointer; padding: 5px 16px;" onclick="miSrn.pss('${secType}', '${stockType}', ${secId}, '', false); miSrn.pc('${secType}', ${secId}, '${stockType}List', '', true, 'inline', true);" oncontextmenu="return false;"> <span class="fas fa-chart-line"></span> Inline </a>`
            html += `       <a class="tsrHmTabBtnIdx" style="cursor:pointer;  padding: 5px 16px;" onclick="miSrn.pss('${secType}', '${stockType}', ${secId}, '', false); miSrn.pc('${secType}', ${secId}, '${stockType}List', '', true, 'tile', true);" oncontextmenu="return false;">  <span class="fas fa-chart-line"></span> Tile  </a>`
            html += `   </div>`
