    ...
    function paintStockSection(params, sectRot, divId) {
        ...
        getDataFromUrl(url).then(data => {
            if (data["statusCode"] == "success") {
                ...
                headerHtml += `<table style="width: 100%; border: 0px; background: linear-gradient(120deg, var(--tsrSecRotBlueDark) 0%, var(--tsrSecRotBlue) 100%);" cellpadding="3" cellspacing="0">
                                    <tbody>
                                        <tr>
                                     ...
                                     <div>${getHeaderHtml(stock, sectRot)}</div>
                                `
                // getHeaderHtml(stock, sectRot);
                let bodyHtml = `<div style="max-height: 70vh; overflow:  auto; border-radius: 10px;">
                            ${getBodyHtml(stock)}
                </div>`;
