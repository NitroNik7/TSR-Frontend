...
function paintStockSection(secType, sector, stockType, secId, stockCode, stock) {
    ...
    // ==========================================
    // 1. POPUP HEADER SECTION
    // ==========================================
    let headerHtml = `
    <div class="px-4 py-3 bg-white border-bottom tsrSecRotHeaderPane">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                <div class="text-center text-md-start">
                    <div class="d-flex align-items-center justify-content-center justify-content-md-start gap-2 mb-1">
                        <span class="badge bg-primary bg-opacity-10 text-primary fw-bold px-2 py-1 small rounded-1 tsrSecRotStockTicker">${stock["id"]}</span>
                        <span class="text-muted small fw-medium tsrSecRotSectorName">${sector["name"] || 'Equity'}</span>
                    </div>
                    <h4 class="fw-bold text-dark m-0 tracking-tight tsrSecRotStockMainTitle">${stock["name"]}</h4>
                </div>
                <div class="d-flex align-items-center gap-3">
                    <a href="${techUrl}" target="_blank" class="btn btn-sm btn-light border text-dark px-3 fw-medium tsrSecRotActionBtn" oncontextmenu="return false;">
                        <i class="fas fa-chart-line text-primary me-2"></i>Technical Analysis
                    </a>
                    <a href="${fundaUrl}" target="_blank" class="btn btn-sm btn-light border text-dark px-3 fw-medium tsrSecRotActionBtn" oncontextmenu="return false;">
                        <i class="fas fa-university text-success me-2"></i>Fundamentals
                    </a>
                </div>
            </div>
        </div>
    ...
    // ==========================================
    // 2. POPUP BODY SECTION (Asymmetric Architecture)
    // ==========================================
    let bodyHtml = `
    <div class="container-fluid p-0 tsrSecRotWorkspace">
        <div class="row g-0">
            
            <!-- LEFT SIDEBAR PANEL: SNAPSHOT METRICS & EMA LIST -->
            <div class="col-12 col-xl-4 bg-light p-4 border-end tsrSecRotSideSummaryPanel">
                
                <!-- KEY HIGHLIGHTS METRIC GRID -->
                <div class="mb-4">
                    <h6 class="text-uppercase tracking-wider text-secondary fw-bold small mb-3"><i class="fas fa-thumbtack me-2 text-primary"></i>Key Highlights</h6>
                    <div class="row g-2">
                        <div class="col-6">
                            <div class="bg-white p-3 rounded border border-light-subtle tsrSecRotMiniOverviewCard">
                                <span class="text-muted d-block tsrSecRotLabelSmall">vs NIFTY</span>
                                <span class="fw-bold fs-5 text-success d-block mt-1">${miSrnUtils.gcv(stock["vsNifty"], null, "%")}</span>
                            </div>
                        </div>`;
                        if (jsu.isNotNull(stock.vsIdx) && stock.vsIdx) {
                            ...
                            <div class="col-6">
                                <div class="bg-white p-3 rounded border border-light-subtle tsrSecRotMiniOverviewCard">
                                    <span class="text-muted d-block tsrSecRotLabelSmall">vs Sector</span>
                                    <span class="fw-bold fs-5 text-success d-block mt-1 text-truncate" title="${sector["name"]}">${miSrnUtils.gcv(stock["vsIdx"], null, "%")}</span>
                                </div>
                            </div>`;
                        }
                        bodyHtml += `
                        <div class="col-6">
                            <div class="bg-white p-3 rounded border border-light-subtle tsrSecRotMiniOverviewCard">
                                <span class="text-muted d-block tsrSecRotLabelSmall">Price</span>
                                <span class="fw-bold fs-5 text-dark d-block mt-1">${miSrnUtils.gcv(stock["price"])}</span>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="bg-white p-3 rounded border border-light-subtle tsrSecRotMiniOverviewCard">
                                <span class="text-muted d-block tsrSecRotLabelSmall">Change</span>
                                <span class="fw-bold fs-5 text-success d-block mt-1">${miSrnUtils.gcv(stock["priceChange"], null, "%")}</span>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- EMA LEDGER STRUCTURE (REPLACED CAROUSEL FOR UX VARIETY) -->
                <div>
                    <h6 class="text-uppercase tracking-wider text-secondary fw-bold small mb-3"><i class="fas fa-wave-square me-2 text-primary"></i>Moving Averages (EMA)</h6>
                    <div class="bg-white rounded border border-light-subtle overflow-hidden shadow-sm tsrSecRotEmaGroupList">`;
                        for (let j = 0; j < stock["ema"].length; j++) {
                            if (jsu.isNull(stock["ema"][j]["label"])) continue;
                            bodyHtml += `
                            <div class="d-flex justify-content-between align-items-center p-2 px-3 border-bottom tsrSecRotEmaRow">
                                <span class="fw-medium text-secondary small m-0">${stock["ema"][j]["label"]}</span>
                                <span class="fw-bold text-dark m-0 fs-6">${miSrnUtils.gcv(stock["ema"][j]["val"], stock["ema"][j]["clrl"], null)}</span>
                            </div>`;
                        }
                    bodyHtml += `
                    </div>
            ...
            <!-- RIGHT CORE DASHBOARD PANEL: HEALTH BLOCKS, RETURNS, INDICATORS -->
            <div class="col-12 col-xl-8 p-4 tsrSecRotCoreDashboard">
                
                <!-- UNIFIED HORIZONTAL SCORE PILLS -->
                <div class="mb-4">
                    <h6 class="text-uppercase tracking-wider text-secondary fw-bold small mb-3"><i class="fas fa-heartbeat me-2 text-danger"></i>TSR Health Scores</h6>
                    <div class="row g-2">
                        <div class="col-6 col-md-4">
                            <div class="p-2 px-3 bg-white border rounded shadow-sm d-flex align-items-center justify-content-between tsrSecRotScorePill">
                                <span class="small fw-medium text-muted">Technical</span>
                                <span class="badge rounded-pill px-2 py-1 text-white small" style="background-color: ${stock["tsrStr"]["techClr"]};">${stock["tsrStr"]["techStr"]}</span>
                            </div>
                        </div>
                        <div class="col-6 col-md-4">
                            <div class="p-2 px-3 bg-white border rounded shadow-sm d-flex align-items-center justify-content-between tsrSecRotScorePill">
                                <span class="small fw-medium text-muted">Value Index</span>
                                <span class="badge rounded-pill px-2 py-1 text-white small" style="background-color: ${stock["tsrStr"]["valClr"]};">${stock["tsrStr"]["valStr"]}</span>
                            </div>
                        </div>
                        <div class="col-6 col-md-4">
                            <div class="p-2 px-3 bg-white border rounded shadow-sm d-flex align-items-center justify-content-between tsrSecRotScorePill">
                                <span class="small fw-medium text-muted">Stability</span>
                                <span class="badge rounded-pill px-2 py-1 text-white small" style="background-color: ${stock["tsrStr"]["stabClr"]};">${stock["tsrStr"]["stabStr"]}</span>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- CAROUSELS & DATA STREAMS -->
                <div class="row g-4">
                    
                    <!-- PERIODIC RETURNS -->
                    <div class="col-12">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <h6 class="text-uppercase tracking-wider text-secondary fw-bold small m-0"><i class="fas fa-history me-2 text-info"></i>Periodic Performance</h6>
                        </div>
                        <div class="owl-carousel owl-theme periodicReturns tsrSecRotDataCarousel text-center">`;
                        for (let j = 0; j < stock["tsrRtn"].length; j++) {
                            if (jsu.isNull(stock["tsrRtn"][j]["label"])) continue;
                            bodyHtml += `
                            <div class="p-3 bg-white border rounded shadow-sm tsrSecRotCarouselMetricItem">
                                <span class="text-muted d-block small mb-1 fw-bold">${stock["tsrRtn"][j]["label"]}</span>
                                <h5 class="fw-bold text-dark m-0 fs-6 text-truncate">${miSrnUtils.gcv(stock["tsrRtn"][j]["stkRtn"], null, "%")}</h5>
                            </div>`;
                        }
                        bodyHtml += `
                        </div>
                    ...
                    <div class="col-12">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <h6 class="text-uppercase tracking-wider text-secondary fw-bold small m-0"><i class="fas fa-bolt me-2 text-warning"></i>Technical Indicators</h6>
                        </div>
                        <div class="owl-carousel owl-theme technicals tsrSecRotDataCarousel text-center">`;
                        for (let j = 0; j < stock["tech"].length; j++) {
                            if (jsu.isNull(stock["tech"][j]["label"])) continue;
                            bodyHtml += `
                            <div class="p-3 bg-white border rounded shadow-sm tsrSecRotCarouselMetricItem">
                                <span class="text-muted d-block small mb-1 fw-bold text-truncate">${stock["tech"][j]["label"]}</span>
                                <h5 class="fw-bold text-dark mb-1 fs-6 text-truncate">${miSrnUtils.gcv(stock["tech"][j]["val"], stock["tech"][j]["clrl"], null)}</h5>
                                <span class="badge bg-opacity-10 small rounded-1 px-2 py-0 fw-medium" style="color: ${stock["tech"][j]["clrl"]}; background-color: ${stock["tech"][j]["clrl"]}1A;">
                                    ${stock["tech"][j]["intr"]}
                                </span>
                            </div>`;
                        }
                    ...
                    <!-- STRATIFIED COMPANY FUNDAMENTALS -->
                    <div class="col-12">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <h6 class="text-uppercase tracking-wider text-secondary fw-bold small m-0"><i class="fas fa-university me-2 text-success"></i>Company Fundamentals</h6>
                        </div>
                        <div class="owl-carousel owl-theme technicals tsrSecRotDataCarousel text-center">`;
                        for (let j = 0; j < stock["funda"].length; j++) {
                            if (jsu.isNull(stock["funda"][j]["label"])) continue;
                            let interpretBadge = stock["funda"][j]["intr"] ? `
                                <span class="badge bg-opacity-10 small rounded-1 px-2 py-0 fw-medium mt-1" style="color: ${stock["funda"][j]["clrl"] || '#6c757d'}; background-color: ${stock["funda"][j]["clrl"] || '#6c757d'}1A;">
                                    ${stock["funda"][j]["intr"]}
                                </span>` : '<span class="text-muted small mt-1 d-block">-</span>';
                            
                            bodyHtml += `
                            <div class="p-3 bg-white border rounded shadow-sm tsrSecRotCarouselMetricItem">
                                <span class="text-muted d-block small mb-1 fw-bold text-truncate" title="${stock["funda"][j]["label"]}">${stock["funda"][j]["label"]}</span>
                                <h5 class="fw-bold text-dark mb-1 fs-6 text-truncate">${miSrnUtils.gcv(stock["funda"][j]["val"], stock["funda"][j]["clrl"], null)}</h5>
                                ${interpretBadge}
                            </div>`;
                        }
                        bodyHtml += `
