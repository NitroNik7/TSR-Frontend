...
var migUi = (function () {  // my Ui Head
	...
	function sideNavDropDownInit() {
		...
		let items = navList.querySelectorAll("li");
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (subMenuList != null) {
				...
				item.addEventListener("touchend", function (e) { // for touch screen devices
					showSubMenuList(e, navMenuCntr, subMenuList, item);
				});
				...
				document.body.addEventListener("touchend", function (e) { // for touch screen devices
					hideSubMenuList(e, navMenuCntr, subMenuList, item);
				});
		...
		function showSubMenuList(e, navMenuCntr, subMenuList, item) {
			...
			navMenuCntr.style.zIndex = "1001"; // ! dangerous - discuss later
		...
		function hideSubMenuList(e, navMenuCntr, subMenuList, item) {
			...
			if (e.type == "touchend") {
				if (!(item == e.target || item.contains(e.target))) { // check if click target is not navItem and is outside subMenu 
		...
		navList.addEventListener('scroll', function (e) {
			if (!isProgramaticScroll) {
				...
				let scrolledHeightFromTop = navList.scrollTop;
				let scrollHeight = actualHeight - visibleHeight;
