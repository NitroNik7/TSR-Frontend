	...
	function sideNavDropDownInit() {
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (subMenuList != null) {
				...
				item.addEventListener("pointerdown", function (e) { // for touch screen devices
		...
		document.body.addEventListener("pointerdown", function (e) {
			...
			for (let i = 0; i < items.length; i++) {
				...
				let subMenuList = item.querySelector("ul"); // sub menu list for above item
					...
					if ((e.target == item || item.contains(e.target)) || subMenuList != null && (e.target == subMenuList || subMenuList.contains(e.target))) {
