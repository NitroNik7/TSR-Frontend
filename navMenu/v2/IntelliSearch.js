var miIs = (function () {  // chart init Params
    ...
    let fotdCanvasId = "tsrFotdCanvas"
    ...
    function init(mobile, type) {
        ...
        if (mobile) {
            ...
            searchForm += `<div id="searchFormDiv">`
            // searchForm += `     <div class="miSearchWrapper d-flex pt-1 w-100" style="box-sizing: border-box;">`
            searchForm += `     <div class="miSearchWrapper d-flex ${typeof fotdJson != "undefined" && jsu.isNotNull(fotdJson) ? "justify-content-around" : ""} w-100" style="box-sizing: border-box;">`
            searchForm += `         <div style="width: auto; text-align: center;">`

            if (!jsu.isMigContext()) {
                searchForm += `             <input id="user_input" type="text" class="form-control " placeholder="Search (Ctrl + K)"`
                // searchForm += `             onclick="miIs.ssb('${searchType}')" autocomplete="off" style="width:200px; margin:10px; margin-bottom: 5px; margin-top: 0px;">`
                searchForm += `             onclick="miIs.ssb('${searchType}')" autocomplete="off" style="width:200px; margin: 3px 10px 0px 10px;">`

                searchForm += `             <a href="${jsu.getBaseUrl()}/rt/TsrHighlights/TsrTechnicalTool" style="margin-left: 10px; font-weight: 500;">India's Best Technical Tool</a>`;
                searchForm += `     </div>`


                // TODO - Remove miIsh.wub()
                // searchForm += `     <div class="miUpdatesBox text-center w-100 mx-3">
                //                         ${topBanner(mobile)}
                //                     </div>`
                searchForm += getFotdHtml();

            } else {
                searchForm += `             <input id = "user_input" type = "text" class="form-control " placeholder = "Search (Ctrl + K)" onclick = "miIs.ssb('${searchType}')" autocomplete = "off" style = "width:200px; margin:10px 0;" > `;
    ...
    */
    ...
    function getFotdHtml() {
        let html = '';
        if (typeof fotdJson != "undefined" && jsu.isNotNull(fotdJson)) {

            // ------------------------------ CSS --------------------------------
            html += `
                                            <style>
                                                /* Base: Hidden for mobile */
                                                .tsrFotdContainer {
                                                        display: none;
                                                }
            ...
            // ------------------------------ HTML --------------------------------
            html += `
                                    <div class="d-flex flex-column justify-content-evenly text-center mx-3">
                                        <div class="tsrFotdContainer" title="Click to know more" data-bs-toggle="offcanvas" data-bs-target="#${fotdCanvasId}" aria-controls="${fotdCanvasId}}" onclick="miIs.pfc('${fotdJson.uri}')">
                                                <div class="tsrFotdBadge" style ='background: #038697;font-weight:600;font-family: sans-serif; font-size:14;'>
                                                    Feature Corner
                                                </div>
                                                <div class="tsrFotdContent">
                                                     <i class="fas fa-arrow-right"></i>
                                                    <span class="tsrFotdText">${fotdJson.title}</span>
                                                </div>
                                        </div>
                                        <div class="offcanvas offcanvas-end" style="min-width: 600px;" tabindex="-1" id="${fotdCanvasId}" aria-labelledby="TSR Feature of the Day Canvas">
                                            <div class="offcanvas-header">
                                                <h5 id="${fotdCanvasId + "Header"}">${fotdJson.title}</h5>
                                                <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                                            </div>
                                            <br>
                                                 <h5> <a href ='${fotdJson.link}'>${fotdJson.subMenu} </a></h5>

                                            <div class="offcanvas-body"  id="${fotdCanvasId + "Body"}">
                                                <div style="align:center;">
                                                    <img src="//www.topstockresearch.com/static/img/LoadingMedium.gif"> 
                                                </div>
                                            </div>
                                        </div>
                                    </div>
    ...
    async function paintFotdCanvas(url) {
        if (jsu.isNull(url)) {
            return;
        }

        let fotdCanvasBodyId = fotdCanvasId + "Body";
        let fotdCanvasBody = document.getElementById(fotdCanvasBodyId);

        // if (jsu.isNotNull(url)) {
        let html = "";
        ...
        if (response.ok) {
            ...
            html = html.replaceAll("STATIC_URL", jsu.getStaticUrl());
            fotdCanvasBody.innerHTML = html;
        }
...
/*
        // feature of the day JSON
        let fotdJson = {
            uri: mintJsUtil.getBaseWwwUrl() + '/static/docs/CustomScreener/price/ohlc/ohlc.html',
            link : mintJsUtil.getBaseWwwUrl() + '/rt/CustomStockScreener.tsr',
            subMenu : 'Custom Screener --> Price Action -->OHLC Compare',
            title: 'OHLC Compare',
        }
